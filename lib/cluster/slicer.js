...
module.exports = function(context) {
    ...
    Promise.all(recoveredSlices).then(function(retryData) {
        ...
        reader.newSlicer(context, job, retryData)
            .then(function(slicers) {
                ...
                throw new Error('Error on slicer initialization: ' + ev)
            });
    }).catch(function(ev) {
        //TODO Need error handling on recovery failure
    });
    ...
    function getRetryData(retryArray, retryObj) {
        ...
        if (retryData.retryList && retryData.retryList.length > 0) {
            ...
            //data has enqueued, remove the rest from memory
            delete retryData.retryList;
        }

        if (retryData.remaining_data) {
            //get the data

            setTimeout(function() {
                //only get more data if slicerQueue has not many slices enqueued
                if (slicerQueue.size() < 500) {
                    state_store.recoveryContext(retryData.job_id, retryData.slicer_id, retryData.remaining_data.count)
                        .then(function(resultObj) {
                            getRetryData(retryArray, resultObj)
                        });
                }
                else {
                    //size of queue is to big, call again
                    getRetryData(retryArray, retryData)
                }
            }, 200)
        }
        else {
            //if all the slices have been allocated and no remaining data left
            if (retryArray.length === 0) {
                event.emit('getRetryData_completed')
            }
            else {
                //still more work to do, recurse to process the next one
