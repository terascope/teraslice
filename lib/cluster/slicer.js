...
module.exports = function(context) {
    ...
    messaging.register('assets:loaded', function(ipcMessage) {
        event.emit('slicer:assets_loaded', ipcMessage)
    });
    ...
    messaging.register('network:disconnect', 'worker_id', function(worker_id) {
        ...
        event.emit('network:disconnect', worker_id);
    ...
    event.once('slice:failure', function() {
    ...
    function slicerInit(retry_Data) {
        ...
        return slicer.newSlicer(context, job, retryData, slicerAnalytics, logger, event)
    }
    ...
    function instantiateJob() {
        return new Promise(function(resolve, reject) {
            ...
            event.on('slicer:assets_loaded', function(ipcMessage) {
                if (ipcMessage.error) {
            ...
            try {
                ...
                //if this errors, then we will wait for the event to fire to start job
            }
        })
    }
