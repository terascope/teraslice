...
module.exports = function(context) {
    ...
    messaging.register('cluster:slicer:analytics', function(msg) {
        logger.trace('api is requesting slicer analytics', slicerAnalytics);
        messaging.respond(msg, {
            message: 'node:message:processed',
    ...
    messaging.register('worker:slice:complete', 'worker_id', function(msg, worker_id) {
        ...
        logger.trace(`worker ${worker_id} has completed its slice,`, msg);
    ...
    function startSlicer(slicerAnalytics) {
        ...
        setTimeout(function() {
            if (!workerFound) {
                logger.debug(`A worker has not connected to a slicer for ex: ${job.jobConfig.ex_id}, shutting down job`)
                messaging.send({
        ...
        Promise.all(recoveredSlices).then(function(retryData) {
            if (retryData.length > 0) {
                logger.debug(`There are ${retryData.length} segments that are being recovered,`, JSON.stringify(retryData))
            }
            return slicer.newSlicer(context, job, retryData, slicerAnalytics, logger)
                .then(function(slicers) {
                    ...
                    //start the engine
                    if (engineCanRun) {
                        logger.trace('starting the slicer engine');
                        engine = setInterval(engineFn, 1);
                    }
                    ...
                    event.on('slicer:job:finished', function() {
                        ...
                        if (slicerDone === totalSlicers) {
                            ...
                            var isDone = setInterval(function() {
                                ...
                                logger.debug(`worker queue: ${workerQueue.size()}, active clients ${messaging.getClientCounts()}, slicer queue: ${slicerQueue.size()}, in recovery mode: ${inRecoveryMode}`);
    ...
    function slicerShutdown(msg) {
        ...
        setInterval(function() {
            ...
            logger.debug(`waiting for connected clients: ${messaging.getClientCounts()} to equal zero`);
