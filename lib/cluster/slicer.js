...
module.exports = function(context) {
    ...
    var logger = context.apis.foundation.makeLogger({module: 'slicer', ex_id: ex_id, job_id: job_id});
                ...
                if (workerQueue.size() >= messaging.getClientCounts() && slicerQueue.size() === 0 && !inRecoveryMode) {
                    ...
                    checkJobState(job.jobConfig)
                        .then(function(errCount) {
                            ...
                            if (errCount > 0) {
                                var message = `job: ${ex_id} had ${errCount} slice failures during processing`;
                        ...
                        .catch(function(err) {
                            ...
                            logger.error(`execution ${ex_id} has run to completion but the process has failed while updating the execution status, slicer will soon exit, error: ${errMsg}`);
    ...
    function slicerRecovery() {
        ...
        if (recoverExecution && job.jobConfig.lifecycle === 'once') {
            ...
            for (var i = 0; i < numOfSlicersToRecover; i++) {
                recoveredSlices.push(state_store.recoveryContext(recoverExecution, i))
            }
        }
