...
module.exports = function(context) {
    ...
    require('./storage/state')(context)
        ...
        .catch(function(err) {
            logger.error(`Slicer: failure during initialization for job ${process.env.job_id}`);
            logger.error(err);
        });
    ...
    io.on('connection', function(socket) {
        ...
        socket.on('worker:ready', function(msg) {
            logger.info(`worker: ${msg.id} has joined slicer: ${job.jobConfig.job_id}`);
        ...
        socket.on('worker:slice_complete', function(msg) {
            ...
            if (msg.retry) {
                logger.warn(`worker: ${msg.id} has rejoined slicer: ${job.jobConfig.job_id}`);
                slicerAnalytics.reconnected_workers += 1;
                socket.teraslice_id = msg.id;
                socket.join(msg.id)
            }

            if (msg.error) {
                logger.error(`worker: ${msg.id} has error on slice: ${JSON.stringify(msg)} , slicer: ${job.jobConfig.job_id}`);
    ...
    function startSlicer(port, slicerAnalytics) {
        ...
        if (process.env.recover_job && job.jobConfig.lifecycle === 'once') {
            logger.info(`slicer: ${job.jobConfig.job_id} is starting in recovery mode`);
        ...
        event.once('slicer:processing_error', function() {
            logger.error(`slicer: ${job.jobConfig.job_id} has encountered a processing_error`);
        ...
        Promise.all(recoveredSlices).then(function(retryData) {
            return reader.newSlicer(context, job, retryData, slicerAnalytics)
                .then(function(slicers) {
                    ...
                    logger.info(`slicer: ${job.jobConfig.job_id} has initialized`);
                    ...
                    event.on('slicer:job_finished', function() {
                        slicerDone++;
                        logger.info(`a slicer for job: ${job.jobConfig.job_id} has completed its range`);

                        if (slicerDone === totalSlicers) {
                            logger.info(`all slicers for job: ${job.jobConfig.job_id} have been completed, 
                            ...
                            var isDone = setInterval(function() {
                                ...
                                if (workerQueue.size() >= Object.keys(io.sockets.sockets).length && slicerQueue.size() === 0) {
                                    logger.info(`all work for job: ${job.jobConfig.job_id} has completed, starting cleanup`);
    ...
    process.on('message', function(msg) {
        ...
        if (msg.message === 'cluster_service:stop_job' || msg.message === 'shutdown') {
            logger.info(`slicer for job: ${job.jobConfig.job_id} has received a shutdown notice`);
        ...
        if (msg.message === 'cluster_service:pause_slicer') {
            logger.info(`slicer for job: ${job.jobConfig.job_id} has received a pause notice`);
            engineCanRun = false;
            clearInterval(engine);
            process.send({
                message: 'message_processed',
        ...
        if (msg.message === 'cluster_service:resume_slicer') {
            logger.info(`slicer for job: ${job.jobConfig.job_id} has received a resume notice`);
            engine = setInterval(engineFn, 1);
            engineCanRun = true;
            process.send({
                message: 'message_processed',
        ...
        if (msg.message === 'cluster_service:restart_slicer') {
            logger.info(`slicer for job: ${job.jobConfig.job_id} has received a restart notice`);
            engineCanRun = false;
            clearInterval(engine);
                    ...
                    .then(function(slice_request) {
                        ...
                        if (slice_request != null) {
                            if (_.isArray(slice_request)) {
                                logger.warn(`slicer for job: ${job.jobConfig.job_id} is subslicing by key`);
