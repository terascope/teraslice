...
module.exports = function(context) {
    ...
    require('./storage/state')(context)
        ...
        .catch(function(err) {
            ...
            logger.error(errMsg);
            messaging.send({message: 'slicer:terminal_error', error: errMsg, ex_id: process.env.ex_id})
        });


    messaging.register('worker:ready', function(msg) {
        workerFound = true;
        logger.info(`worker: ${msg.id} has joined slicer: ${job.jobConfig.ex_id}`);
        slicerAnalytics.workers_joined += 1;
        messaging.decorateConnection('teraslice_id', msg.id);
        messaging.joinRoom(msg.id);
        workerQueue.enqueue(msg);
    });

    messaging.register('worker:slice:complete', function(msg) {
        ...
        if (msg.error) {
            ...
            if (inRecoveryMode) {
                messaging.send({message: 'slicer:failed_recovery', ex_id: job.jobConfig.ex_id, transmit: true});
            }
        }
        else {
            if (msg.analytics) {
    ...
    function startSlicer(port, slicerAnalytics) {
        ...
        event.once('slicer:processing_error', function() {
            ...
            messaging.send({message: 'slicer:processing_error', ex_id: job.jobConfig.ex_id, transmit: true});
        });
                ...
                .then(function(slicers) {
                    ...
                    engineFn = function engineFn() {
                        ...
                        while (workerQueue.size() && slicerQueue.size()) {
                            ...
                            messaging.send(worker.id, 'slicer:slice:new', {message: 'data', data: sliceData});
                        }
                    ...
                    messaging.send({message: 'slicer:slicer_initialized', ex_id: job.jobConfig.ex_id, transmit: true});
                    ...
                    event.on('slicer:job:finished', function() {
                        ...
                        if (slicerDone === totalSlicers) {
                            ...
                            var isDone = setInterval(function() {
                                ...
                                if (workerQueue.size() >= messaging.getClientCounts() && slicerQueue.size() === 0) {
    ...
    messaging.register('worker:message', function(msg) {

        if (msg.message === 'cluster:job:stop' || msg.message === 'shutdown') {
        ...
        if (msg.message === 'cluster:job:pause') {
        ...
        if (msg.message === 'cluster:job:resume') {
        ...
        if (msg.message === 'cluster:job:restart') {
        ...
        if (msg.message === 'cluster:slicer:analytics') {
            messaging.send({
                message: 'node:message:processed',
    ...
    messaging.register('SIGTERM', noOP);
    messaging.register('SIGINT', noOP);
    ...
    function terminalShutdown(errEV) {
        ...
        messaging.send({message: 'job:terminal_error', error: errEV.err, ex_id: process.env.ex_id, transmit: true})
    }
