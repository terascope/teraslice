...
module.exports = function(context) {
    ...
    messaging.register('worker:slice:complete', function(msg) {
        ...
        if (msg.error) {
            ...
            if (inRecoveryMode) {
                messaging.send({message: 'slicer:failed_recovery', ex_id: job.jobConfig.ex_id});
            }
        }
        else {
            if (msg.analytics) {
    ...
    function startSlicer(port, slicerAnalytics) {
        ...
        event.once('slicer:processing_error', function() {
            ...
            messaging.send({message: 'slicer:processing_error', ex_id: job.jobConfig.ex_id});
        });
        ...
        Promise.all(recoveredSlices).then(function(retryData) {
            return slicer.newSlicer(context, job, retryData, slicerAnalytics, logger)
                .then(function(slicers) {
                    ...
                    messaging.send({message: 'slicer:slicer_initialized', ex_id: job.jobConfig.ex_id});
    ...
    function terminalShutdown(errEV) {
        ...
        messaging.send({message: 'job:terminal_error', error: errEV.err, ex_id: process.env.ex_id})
    }
