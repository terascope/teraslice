...
module.exports = function module(contextConfig) {
    ...
    messaging.register('cluster:job:pause', (msg) => {
    ...
    messaging.register('cluster:job:resume', (msg) => {
    ...
    // TODO: what is this doing, who is emitting this?
    messaging.register('cluster:job:restart', (msg) => {
        logger.info(`slicer for job: ${exId} has received a restart notice`);
        engineCanRun = false;
        clearInterval(engine);
    ...
    messaging.register('cluster:slicer:analytics', (msg) => {
        ...
        messaging.respond(msg, {
            ...
            data: { node_id: msg.node_id, name, job_id: jobId, ex_id: exId, stats: slicerAnalytics }
        });
    });
    ...
    messaging.register('worker:ready', 'worker_id', (msg, workerId) => {
        workerFound = true;
    ...
    messaging.register('worker:slice:complete', 'worker_id', (msg, workerId) => {
        ...
        if (msg.error) {
            logger.error(`worker: ${workerId} has error on slice: ${JSON.stringify(msg)} , slicer: ${exId}`);
            ...
            if (inRecoveryMode) {
                ...
                messaging.send({ to: 'cluster_master', message: 'slicer:recovery:failed', ex_id: exId, reply: false });
        ...
        logger.debug(`worker ${workerId} has completed its slice,`, msg);

        // Report to worker that it has been reported, so it can remove reference to last slice
        messaging.send({ to: workerId, message: 'slicer:slice:recorded', payload: null, reply: false });
        if (!msg.isShuttingDown) {
            workerQueue.enqueue(msg);
        }
    });

    messaging.register('network:disconnect', 'worker_id', (workerId) => {
    ...
    if (process.env.__process_restart) {
        ...
        messaging.send({ to: 'cluster_master', message: 'job:error:terminal', error: errMsg, ex_id: exId, reply: false });
    } else {
    ...
    function slicerRecovery() {
        ...
        setTimeout(() => {
            if (!workerFound) {
                ...
                const errorMeta = exStore.failureMeta('No workers have connected to slicer in allotted time', slicerAnalytics);
                exStore.setStatus(exId, 'failed', errorMeta)
                    .then(() => {
                        messaging.send({
                            to: 'cluster_master',
    ...
    function slicerEngine(slicers) {
        ...
        engineFn = function slicerEngineExecution() {
            while (workerQueue.size() && slicerQueue.size()) {
                ...
                messaging.send({ to: worker.worker_id, message: 'slicer:slice:new', payload: sliceData, reply: false });
            }
    ...
    function pushAnalytics() {
        ...
        messaging.send({ to: 'cluster_master', message: 'cluster:analytics', stats: diffs, kind: 'slicer', reply: false });
    ...
    function terminalShutdown(errEV) {
        ...
        messaging.send({ to: 'cluster_master', message: 'job:error:terminal', error: errEV.err, ex_id: exId, reply: false });
    }
