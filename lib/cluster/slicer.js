...
module.exports = function(context) {
    ...
    require('./storage/state')(context)
        ...
        .catch(function(err) {
            logger.error(`Slicer: failure during initialization for job ${process.env.jx_id}`);
            logger.error(err);
        });
    ...
    io.on('connection', function(socket) {
        ...
        socket.on('worker:ready', function(msg) {
            logger.info(`worker: ${msg.id} has joined slicer: ${job.jobConfig.jx_id}`);
        ...
        socket.on('worker:slice_complete', function(msg) {
            ...
            if (msg.error) {
                logger.error(`worker: ${msg.id} has error on slice: ${JSON.stringify(msg)} , slicer: ${job.jobConfig.jx_id}`);
                slicerAnalytics.failed += 1;
                event.emit('slicer:processing_error');
                //if an error occurred while in recovery, fail the job as a whole
                if (inRecoveryMode) {
                    process.send({message: 'slicer:failed_recovery', jx_id: job.jobConfig.jx_id});
                }
            }
            else {
                if (msg.analytics) {
    ...
    function startSlicer(port, slicerAnalytics) {
        ...
        if (process.env.recover_job && job.jobConfig.lifecycle === 'once') {
            logger.info(`slicer: ${job.jobConfig.jx_id} is starting in recovery mode`);

            inRecoveryMode = true;

            var numOfSlicersToRecover = slicer.parallelSlicers ? job.jobConfig.slicers : 1;
            for (var i = 0; i < numOfSlicersToRecover; i++) {
                recoveredSlices.push(state_store.recoveryContext(job.jobConfig.jx_id, i))
            }
        }
        ...
        event.on('getRetryData_completed', function() {
            logger.info(`The recovered data for job: ${job.jobConfig.jx_id} has successfully been enqueued`);
            inRecoveryMode = false;
        });

        event.once('slicer:processing_error', function() {
            logger.error(`slicer: ${job.jobConfig.jx_id} has encountered a processing_error`);
            process.send({message: 'slicer:processing_error', jx_id: job.jobConfig.jx_id});
        });
        ...
        Promise.all(recoveredSlices).then(function(retryData) {
            return slicer.newSlicer(context, job, retryData, slicerAnalytics)
                .then(function(slicers) {
                    ...
                    logger.info(`slicer: ${job.jobConfig.jx_id} has initialized`);
                    process.send({message: 'slicer:slicer_initialized', jx_id: job.jobConfig.jx_id});
                    ...
                    event.on('slicer:job_finished', function() {
                        slicerDone++;
                        logger.info(`a slicer for job: ${job.jobConfig.jx_id} has completed its range`);

                        if (slicerDone === totalSlicers) {
                            logger.info(`all slicers for job: ${job.jobConfig.jx_id} have been completed, 
                            ...
                            var isDone = setInterval(function() {
                                ...
                                if (workerQueue.size() >= Object.keys(io.sockets.sockets).length && slicerQueue.size() === 0) {
                                    logger.info(`all work for job: ${job.jobConfig.jx_id} has completed, starting cleanup`);
            ...
            process.send({message: 'slicer:error', error: errorMsg, jx_id: process.env.jx_id})
        });
    }
    ...
    process.on('message', function(msg) {
        ...
        if (msg.message === 'cluster_service:stop_job' || msg.message === 'shutdown') {
            logger.info(`slicer for job: ${job.jobConfig.jx_id} has received a shutdown notice`);
        ...
        if (msg.message === 'cluster_service:pause_slicer') {
            logger.info(`slicer for job: ${job.jobConfig.jx_id} has received a pause notice`);
            engineCanRun = false;
            clearInterval(engine);
            process.send({
                message: 'message_processed',
        ...
        if (msg.message === 'cluster_service:resume_slicer') {
            logger.info(`slicer for job: ${job.jobConfig.jx_id} has received a resume notice`);
            engine = setInterval(engineFn, 1);
            engineCanRun = true;
            process.send({
                message: 'message_processed',
        ...
        if (msg.message === 'cluster_service:restart_slicer') {
            logger.info(`slicer for job: ${job.jobConfig.jx_id} has received a restart notice`);
            engineCanRun = false;
            clearInterval(engine);
        ...
        if (msg.message === 'cluster_service:slicer_analytics') {
            process.send({
                ...
                _msgID: msg._msgID,
                data: {node_id: msg.node_id, jx_id: job.jobConfig.jx_id, stats: slicerAnalytics}
            });
            return;
        }
    });
    ...
    function getRetryData(retryArray, retryObj) {
        ...
        var retrySlicer = setInterval(function() {
            ...
            if (slicerQueue.size() === 0 && Object.keys(retryState).length === 0 && !isGettingData) {
                ...
                state_store.recoveryContext(retryData.jx_id, retryData.slicer_id)
    ...
    function checkJobState(jobConfig) {
        if (jobConfig.lifecycle !== "persistent") {
            var query = 'jx_id:' + jobConfig.jx_id + ' AND state:error';
                    ...
                    .then(function(slice_request) {
                        ...
                        if (slice_request != null) {
                            if (_.isArray(slice_request)) {
                                logger.warn(`slicer for job: ${job.jobConfig.jx_id} is subslicing by key`);
