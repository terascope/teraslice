...
module.exports = function(context) {
    ...
    event.on('slicer:job:finished', function() {
        ...
        if (slicerDone === totalSlicers) {
            logger.info(`all slicers for job: ${job.jobConfig.ex_id} have been completed, waiting for slices in slicerQueue to be processed`);
    ...
    if (process.env.__process_restart) {
        var errMsg = `Slicer for ex_id: ${job.jobConfig.ex_id} runtime error led to a restart, terminating job with failed status, please use the recover api to return slicer to a consistent state`;
        logger.error(errMsg);
        messaging.send({message: 'slicer:error:terminal', error: errMsg, ex_id: job.jobConfig.ex_id})
    }
    else {
                ...
                slicerInit()
                    ...
                    .catch(function(err) {
                        ...
                        if (times < max_retries) {
                            ...
                            var retryErrMsg = elasticError(err);
                    ...
                    .then(function(resultObj) {
                        ...
                        if (resultObj.retryList.length > 0) {
                            ...
                            if (retryArray.length === 0) {
                                event.emit('slicer:recovered_data_enqueued')
                            }
                            else {
            ...
            if (!isProcessing) {
                ...
                Promise.resolve(slicer())
                    ...
                    .catch(function(err) {
                        ...
                        isProcessing = false;
                        var errMsg = elasticError(err);
                        logger.error(`slicer for ex ${process.env.ex_id} had an error, still continuing`, errMsg);
                    })
            ...
            if (!isProcessing) {
                ...
                Promise.resolve(slicer())
                    ...
                    .catch(function(err) {
                        ...
                        isProcessing = false;
                        var errMsg = elasticError(err);
                        logger.error(`slicer for ex ${process.env.ex_id} had an error, shutting down job`, errMsg);
                        messaging.send({message: 'job:error:terminal', error: errMsg, ex_id: process.env.ex_id})
                    })
            }
        }
    }
