...
module.exports = function(context) {
    ...
    function persistent(slicer, slicerQueue, job) {
    ...
    function once(slicer, slicerQueue, job) {
    ...
    function getScheduler(job, slicers, slicerQueue) {
        ...
        slicers.forEach(function(slicer) {
            fnArray.push(lifecycle(job.jobConfig)(slicer, slicerQueue, job));
        });
    ...
    Promise.resolve(job_runner.initialize())
        .then(function(job) {
            ...
            reader.newSlicer(context, job.readerConfig, job.jobConfig).then(function(slicers) {
                ...
                engineFn = function engineFn() {
                    ...
                    while (workerQueue.size() && slicerQueue.size()) {
                        ...
                        if (retryQueue && retryQueue.size()) {
                            ...
                            // TODO: Need to verify the addition of slice_id didn't break this
                    ...
                    //don't run if all slices have been divided up or if the queue is too large
                    if (!allSlicersDone && slicerQueue.size() <= 10000) {
