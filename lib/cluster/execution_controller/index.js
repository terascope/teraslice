...
module.exports = function module(contextConfig) {
    ...
    events.once('slice:failure', () => {
        ...
        const errorMeta = exStore.failureMeta(null, executionStats);
    ...
    function executionInit() {
        ...
        return Promise.resolve(require('../storage/assets')(context))
            ...
            .spread((stateStorage, exStorage) => {
                ...
                slicerAnalytics = require('./execution_analytics')(executionModules);
                executionModules.slicerAnalytics = executionAnalytics;
    ...
    // TODO refactor watchDog, they share commonality
    ...
    function workerConnectionWatchDog() {
        ...
        setTimeout(() => {
            if (!workerFound) {
                ...
                const executionStats = executionAnalytics.getAnalytics();
                const errorMeta = exStore.failureMeta(`No workers have connected to slicer in the allotted time: ${slicerTimeout} ms`, executionStats);
                exStore.setStatus(exId, 'failed', errorMeta)
                    .then(() => {
                        messaging.send({
                            to: 'cluster_master',
