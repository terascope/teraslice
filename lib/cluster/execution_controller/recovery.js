...
function recovery(context, terminalError, stateStore, executionContext) {
    ...
    function initialize() {
        ...
        events.once('slice:failure', terminalError);
        ...
        events.once('execution:recovery:complete', () => {
            events.removeListener('slice:failure', terminalError);
        ...
        return new Promise((resolve, reject) => {
            function retrieveSlices(slicerID) {
                ...
                _processIncompleteSlices(slicerID)
                    ...
                    .catch(err => reject(new VError(err, 'failed to recover slicer_id %s', slicerID)));
            }
            ...
            return getSlicerStartingPosition()
                .then((executionStartingPoints) => {
                    events.emit('execution:recovery:complete', executionStartingPoints);
                }).catch((err) => {
                    const error = new VError(err, 'Failed to get recovery starting points');
