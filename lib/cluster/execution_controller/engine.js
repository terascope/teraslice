...
module.exports = function module(executionModules) {
    ...
    const slicerAnalytics = executionAnalytics.slicerAnalytics;
            ...
            if (!isProcessing && !hasCompleted) {
                ...
                Promise.resolve(slicerFn())
                    ...
                    .catch((err) => {
                        ...
                        const errorMeta = exStore.failureMeta(errMsg, executionStats);
                        logger.error(errMsg);
        ...
        return new Promise((resolve, reject) => {
            function retrieveSlices(slicerID) {
                fetchSlicesBySlicerID(slicerID)
                    .then((slices) => {
                        console.log('whats coming out of here', slices);
                        if (slices.length === 0) {
                    ...
                    checkJobState(executionConfig)
                        .then((errCount) => {
                            ...
                            if (errCount > 0) {
                                ...
                                const errorMeta = exStore.failureMeta(message, executionStats);
                                logger.error(message);
                                exStore.setStatus(exId, 'failed', errorMeta);
                            } else {
                                // TODO review this in regards to failureMeta fn
                                logger.info(`execution ${exId} has completed`);
                                const metaData = { _slicer_stats: executionStats };
                                if (hasRecovered) {
