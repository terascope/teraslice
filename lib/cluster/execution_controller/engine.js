...
module.exports = function module(context, messaging, exStore, stateStore) {
    ...
    messaging.register({
        ...
        callback: (msg, workerId) => {
            ...
            const { slice_id: sliceId, slice_order: sliceOrder } = workerResponse;

            const id = `${workerId}:${sliceId}:${sliceOrder}`;
            const alreadyCompleted = _.has(_completedSlicesCache, id);
            const shouldEnqueue = !workerResponse.isShuttingDown;

            _.set(_completedSlicesCache, id, true);
            ...
            if (alreadyCompleted) {
                logger.warn(`worker: ${workerId} already marked this slice as complete`);
            } else {
    ...
    messaging.register({
        ...
        callback: (msg, workerId) => {
            ...
            logger.warn(`Worker: ${workerId} has disconnected for reason ${msg}`);
    ...
    function _sendSlice(sliceData, address) {
        logger.info(`Sending slice to ${address}`);
        const messageData = {
            to: 'worker',
            address,
        ...
        return messaging.send(messageData)
            .then((res) => {
                ...
                if (workerResponse === null) return Promise.reject('worker is not responding back with correctly formatted message');
    ...
    function testContext(_executionContext, _executionAnalytics, _slicerAnalytics, _recovery, _exId, _testRecovery) {
        isTesting = true;
        _completedSlicesCache = {};
