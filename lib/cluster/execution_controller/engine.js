...
module.exports = function module(context, messaging, exStore, stateStore) {
    ...
    messaging.register({
        ...
        callback: (msg, workerId) => {
            ...
            if (!sliceData.isShuttingDown) {
                _enqueueWorker(sliceData);
            }
        }
    });

    messaging.register({
        event: 'worker:slice:over_allocated',
        callback: (workerResponse) => {
            logger.error(`worker ${workerResponse.worker_id} was given another slice before finishing current work, re-enqueueing slice`);
            slicerQueue.unshift(workerResponse.payload);
        }
    });
    ...
    function _enqueueWorker(worker) {
        if (worker == null) {
            throw new Error('Failed to enqueue null as a worker');
        }
