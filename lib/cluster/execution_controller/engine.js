...
module.exports = function module(context, messaging, exStore, stateStore) {
    ...
    function _sendSlice(sliceData, address) {
        ...
        return messaging.send(messageData)
            .then((res) => {
                const workerResponse = res.payload;
                if (!workerResponse.willProcess) {
                    logger.error(`worker ${address} was given another slice before finishing current work, re-enqueueing slice`);
