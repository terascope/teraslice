...
const VError = require('@terascope/error-parser');
...
module.exports = function module(context, messaging, exStore, stateStore, config = {}) {
        ...
        return new Promise((resolve, reject) => {
            if (recoverExecution) {
                ...
                Promise.resolve()
                    ...
                    .catch((err) => {
                        const error = new VError(err, 'failed to register recovery slicers');
                        reject(error);
                    });
            } else {
                resolve([]);
    ...
    function _slicerInitRetry(slicerError) {
        ...
        const error = new VError({
            name: 'SliceRetryError',
            cause: slicerError,
            info: {
                ex_id: exId,
                job_id: jobId
            }
        }, 'failed slicer initialization, will attempt to retry %d times', maxRetries);
                ...
                _slicerInit(startingPointStateRecords)
                    ...
                    .catch((err) => {
                        ...
                        if (times < maxRetries) {
                            ...
                            const retryError = new VError({
                                name: 'SlicerInitError',
                                cause: err,
                                info: {
                                    attempts: times,
                                    job_id: jobId,
    ...
    function terminalError(err) {
        const error = new VError({
            name: 'ExecutionTerminalError',
            cause: err,
            info: {
                job_id: jobId,
                ex_id: exId,
            }
        }, 'execution_controller terminal failure');
    ...
    function _sendSlice(sliceData, address) {
        ...
        return messaging.send(messageData)
            ...
            .catch((err) => {
                const error = new VError(err, 'failed waiting for response from slice allocation %s', address);
                logError(logger, error);
            });
    }
            ...
            if (!isProcessing && !hasCompleted) {
                ...
                Promise.resolve()
                    ...
                    .catch((err) => {
                        ...
                        const error = new VError(err, 'slicer for ex %s had an error, shutting down execution', exId);
                        const executionStats = executionAnalytics.getAnalytics();
                        const errorMeta = exStore.executionMetaData(executionStats, error.stack);
                        logError(logger, error);
    ...
    function shutdown() {
        ...
        return Promise.resolve()
            ...
            .then(() => logger.flush())
            .catch((err) => {
                const error = new VError(err, 'failed to shutdown execution');
                logError(logger, error);
            });
    }
    ...
    function _executionCompleted() {
        ...
        _checkExecutionState(executionConfig)
            ...
            .catch((err) => {
                const error = new VError(err, 'execution %s has run to completion but the process has failed while updating the execution status, slicer will soon exit', exId);
                logError(logger, error);
