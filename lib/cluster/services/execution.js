...
module.exports = function module(context) {
    ...
    function shutdown() {
        ...
        return searchExecutionContexts(query)
            ...
            .catch((err) => {
                const error = new VError(err, 'failure while shutting down execution stores');
    ...
    function createExecutionContext(job) {
        return exStore.create(job, 'ex')
            .then(ex => setExecutionStatus(ex.ex_id, 'pending')
                ...
                .catch((err) => {
                    const error = new VError(err, 'could not set to pending');
                    return Promise.reject(error);
                }))
            .catch((err) => {
                const error = new VError(err, 'could not create execution context');
                logError(logger, error);
                return Promise.reject(error);
            });
    }
    ...
    function getExecutionContext(exId) {
        if (!exId) {
            const error = new Error('Unable to get execution with missing exId');
        ...
        return exStore.get(exId)
            .catch((err) => {
                const error = new VError(err, 'failure getting execution context for ex: %s', exId);
                return Promise.reject(error);
            });
    }
    ...
    function getActiveExecution(exId) {
        ...
        return searchExecutionContexts(query, null, 1, '_created:desc')
            .then((ex) => {
                if (ex.length === 0) {
                    const error = new VError(`no active execution context was found for ex_id: ${exId}`);
    ...
    function recoverExecution(exId, cleanup) {
        return getExecutionContext(exId)
            ...
            .catch((err) => {
                const error = new VError(err, 'Unable to recover execution');
                return Promise.reject(error);
            });
    }
                    ...
                    .then(() => allocateSlicer(execution)
                        ...
                        .then(() => allocateWorkers(execution, execution.workers)
                            .catch((err) => {
                                ...
                                const error = new VError(err, 'Workers failed to be allocated, they will be enqueued');
                                logError(logger, error);
                            })))
                    .catch((err) => {
                        const error = new VError(err, 'Failed to provisiion execution %s', execution.ex_id);
                        logError(logger, error);
        ...
        return searchExecutionContexts('running').each((execution) => {
            ...
            .error((err) => {
                ...
                if (_.get(err, 'body.error.reason') !== 'no such index') {
                    const error = new VError(err, 'initialization failed loading state from Elasticsearch');
                    logError(logger, error);
                    return Promise.reject(error);
                }

                const error = new VError(err, 'failure to initialize execution');
                logError(logger, error);
                return Promise.reject(error);
            });
    }
