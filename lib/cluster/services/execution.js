...
module.exports = function module(context, server) {
    ...
    // TODO: investigate the guard logic, should all jobs be marked as failed?
    ...
    // TODO: better comments on what is going on
    function _cleanUpNode(nodeId) {
        ...
        _.forIn(node.workerJobID, (__, exId) => {
            ...
            if (!node.slicerJobID[exId]) {
                ...
                // FIXME: this does not work, need jobID
                Promise.all([
                    getExecutionContext(exId),
                    isExecutionActive(exId)
                ])
    ...
    // TODO: need to change name to execution
        ...
        return searchExecutionContexts(query)
            .map((job) => {
                if (getClusterType === 'native') {
                    ...
                    // context.sysconfig.teraslice.hostname);
                }
                return true;
            })
    ...
    // TODO: rename this more consistently
