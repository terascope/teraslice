...
module.exports = function module(context) {
    ...
    function createExecutionContext(job) {
        return exStore.create(job, 'ex')
            .then(ex => setExecutionStatus(ex.ex_id, 'pending')
                ...
                .catch((err) => {
                    const errMsg = parseError(err);
                    const error = new Error(`could not set to pending ${errMsg}`);
                    logger.error(error.message);
                    return Promise.reject(error);
                }))
            .catch((err) => {
                const errMsg = parseError(err);
                const error = new Error(`could not create execution context ${errMsg}`);
                logger.error(error.message);
                return Promise.reject(error);
            });
    }
    ...
    function getExecutionContext(exId) {
        ...
        return exStore.get(exId)
            .catch((err) => {
                const errMsg = parseError(err);
                const error = new Error(`error getting execution context for ex: ${exId} ${errMsg}`);
                logger.error(error);
                return Promise.reject(error);
            });
    }
    ...
    function recoverExecution(exId, cleanup) {
        return getExecutionContext(exId)
            ...
            .catch((err) => {
                ...
                const error = new Error(`Unable to recover execution ${errorMsg}`);
                return Promise.reject(error);
            });
    }
    ...
    function _initialize() {
        ...
        return searchExecutionContexts('running').each((execution) => {
            ...
            // TODO: verify whats coming here
                ...
                const errMsg = parseError(err);
                const error = new Error(`Error initializing, ${errMsg}`);
                logger.error(error);
                return Promise.reject(error);
            });
    }
