...
module.exports = function module(context, app) {
    ...
    v1routes.get('/jobs', (req, res) => {
        ...
        const startingQuery = 'job_id:*';
        search(logger, req, startingQuery, jobsService.searchJobs)
            .then((results) => {
    ...
    v1routes.put('/jobs/:job_id', (req, res) => {
        const { body: jobSpec, params: { job_id: jobId } } = req;
    ...
    v1routes.get('/jobs/:job_id/errors/', (req, res) => {
        ...
        jobsService.getLatestExecutionId(jobId)
            .then((exId) => {
                ...
                const startingQuery = `state:error AND ex_id:${exId}`;
                return search(logger, req, startingQuery, stateStore.search);
            })
    ...
    v1routes.get('/jobs/:job_id/errors/:ex_id', (req, res) => {
        const { params: { job_id: jobId, ex_id: exId } } = req;

        const handleApiError = handleError(res, logger, 500, `Could not get errors for job: ${jobId}, execution: ${exId}`);
        const startingQuery = `ex_id:${exId} AND state:error`;

        search(logger, req, startingQuery, stateStore.search)
            .then((errorStates) => {
    ...
    v1routes.get('/ex', (req, res) => {
        const handleApiError = handleError(res, logger, 500, 'Could not retrieve list of execution contexts');
        const startingQuery = 'ex_id:*';

        search(logger, req, startingQuery, executionService.searchExecutionContexts)
            .then(results => res.status(200).json(results))
            .catch(handleApiError);
    });
    ...
    app.get('/txt/jobs', (req, res) => {
        ...
        search(logger, req, null, jobsService.searchJobs)
            .then((jobs) => {
