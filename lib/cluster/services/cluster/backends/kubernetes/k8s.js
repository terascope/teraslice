    ...
    */
    ...
    async delete(name, objType) {
        ...
        try {
            ...
            this.logger.error(`Error trying to delete ${name}: ${e}`);
            // FIXME: What is the appropriate way to handle errors in this case?
        }

        if (![200, 202, 204].includes(response.statusCode)) {
            const msg = `Unexpected response code, ${response.statusCode}, when deleting, ${name}`;
            this.logger.error(msg);
        }
    ...
    async _deleteObjByExId(exId, nodeType, objType) {
        ...
        this.logger.info(`k8s ${nodeType} ${objType} query response: ${objList.items[0].metadata.name}`);
    ...
    async scaleExecution(exId, numWorkers, op) {
        ...
        // FIXME: This returns after waiting on the k8s stuff, should it?  I
        // think it's OK.  Technically it's returning after the deployment scale
        // has been patched but doesn't block until the scaling is actually
        // complete, that is still done out of band by k8s
