...
module.exports = function kubernetesClusterBackend(context, messaging) {
    ...
    function allocateSlicer(execution, recoverExecution) {
        const name = `teraslice-slicer-${execution.ex_id}`.substring(0, 63);
        ...
        const slicerService = jsonTemplater(slicerServiceTemplate, serviceConfig);

        execution.slicer_port = _.get(slicerService, 'spec.ports[0].targetPort');
        execution.slicer_hostname = _.get(slicerService, 'metadata.name');
        ...
        const slicerDeployment = jsonTemplater(slicerDeploymentTemplate, deploymentConfig);
