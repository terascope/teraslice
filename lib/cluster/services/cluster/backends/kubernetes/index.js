...
module.exports = function kubernetesClusterBackend(context, messaging, executionService) {
    ...
    function _scaleDeployment(exId, numWorkers, op) {
        ...
        k8s.getObj(`nodeType=worker,exId=${exId}`, objType)
            .then((response) => {
                ...
                logger.warn(`Current Scale: ${originalScale}`);
                ...
                logger.warn(`New Scale for ${exId}: ${newScale}`);
                const p = {
                    spec: {
                        replicas: newScale
                    }
                };
                k8s.patch(p, r.metadata.name)
                    .then(r1 => logger.warn(r1));
            })
            .catch(err => logger.error(`The cause of great sadness: ${err}`));
    ...
    function removeWorkers(executionContext, numWorkers) {
        return _scaleDeployment(executionContext.ex_id, numWorkers, 'remove');
    }
