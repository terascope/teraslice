...
const exJobTemplate = require('./jobs/execution_controller.json');
...
module.exports = function kubernetesClusterBackend(context, messaging) {
    ...
    // FIXME: The default timeout should already be applied to the config,
    // should we assume it's there, or supply a default like I do here, which is
    // technically duplication
    const shutdownTimeoutSeconds = _.get(context, 'sysconfig.teraslice.shutdown_timeout', 60000) / 1000;
    ...
    function allocateSlicer(execution, recoverExecution) {
        ...
        const exJob = jsonTemplater(exJobTemplate, jobConfig);

        // FIXME: jsonTemplater can't handle numeric substitutions
        // https://github.com/lightsofapollo/json-templater/blob/master/object.js#L39
        exJob.spec.template.spec.terminationGracePeriodSeconds = shutdownTimeoutSeconds;
    ...
    function allocateWorkers(execution, numWorkers) {
        ...
        // FIXME: jsonTemplater can't handle numeric substitutions
        // https://github.com/lightsofapollo/json-templater/blob/master/object.js#L39
        workerDeployment.spec.template.spec.terminationGracePeriodSeconds = shutdownTimeoutSeconds;
