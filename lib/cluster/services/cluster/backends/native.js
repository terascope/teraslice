...
module.exports = function module(context, messaging, executionService) {
    ...
    messaging.register('slicer:execution:finished', (msgData) => {
    ...
    messaging.register('execution:error:terminal', (msgData) => {
        ...
        return _findPort(slicerNodeID)
            .then((portObj) => {
                ...
                return messaging.send({
                    ...
                    message: 'cluster:slicer:create',
                    payload: data,
                    response: true
                })
                    .catch((err) => {
                        const errMsg = parseError(err);
                        logger.error(`failed to allocate slicer to ${slicerNodeID}, error: ${errMsg}`);
                ...
                _createSlicer(job, recoverExecution, errorNodes)
                    ...
                    .catch(() => {
                        ...
                        if (retryCount >= slicerAllocationAttempts) {
                            reject(`failed to allocate slicer to nodes: ${JSON.stringify(errorNodes)}`);
                        } else {
                            retry(errorNodes);
                        }
                    });
