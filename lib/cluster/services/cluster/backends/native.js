...
module.exports = function module(context, messaging, executionService) {
            ...
            .then((portObj) => {
                ...
                return messaging.send({
                    ...
                    .catch((err) => {
                        ...
                        const error = new Error(`failed to allocate execution_controller to ${slicerNodeID}, error: ${errMsg}`);
                        logger.error(error);
    ...
    function removeWorkers(exId, workerNum) {
        ...
        return Promise.all(results)
            ...
            .catch((err) => {
                const errMsg = parseError(err);
                const error = new Error(`Error while releasing workers from job ${exId}, error: ${errMsg}`);
                logger.error(error);
                return Promise.reject(error);
            });
    }
    ...
    function _notifyNodesWithExecution(exId, messageData, slicerOnly, excludeNode) {
        return new Promise(((resolve, reject) => {
            ...
            Promise.map(nodes, (node) => {
                ...
                .catch((err) => {
                    const errMsg = parseError(err);
                    const error = new Error(`could not notify cluster ${errMsg}`);
