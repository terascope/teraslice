...
module.exports = function module(context, messaging, executionService) {
    ...
    // TODO this will become unneeded in message refactor, needs to be internalized
    messaging.register('node:message:processed', (data) => {
        // emitting the unique msg id to allow easier listener cleanup
        //  console.log('im at the node:message:processed', data);
    ...
    messaging.register('slicer:job:finished', (msgData) => {
        // remove any pending worker request on completed jobs
        logger.debug(`job for ex_id: ${msgData.ex_id} has finished, removing any from pending queue`);
        logger.trace(`job ex_id: ${msgData.ex_id} has finished,  message:`, msgData);
        pendingWorkerRequests.remove(msgData.ex_id);
        messaging.broadcast('cluster:job:stop', msgData);
    });

    messaging.register('slicer:recovery:failed', (msgData) => {
        // remove any pending worker request
        logger.debug(`slicer for execution: ${msgData.ex_id} has failed on recovery`);
        pendingWorkerRequests.remove(msgData.ex_id);
        messaging.broadcast('cluster:job:stop', { ex_id: msgData.ex_id });
    });

    messaging.register('job:error:terminal', (msgData) => {
        logger.debug(`terminal job error for execution: ${msgData.ex_id}, canceling job`);
        const metaData = _errorMetaData(msgData);
        pendingWorkerRequests.remove(msgData.ex_id);
        messaging.broadcast('cluster:job:stop', { ex_id: msgData.ex_id });
    ...
    function _cleanUpNode(nodeId) {
        ...
        if (node.hasSlicer) {
            _.forIn(node.slicerExecutions, (exId) => {
                ...
                // TODO: should we target nodes or blanket broadcast?
                messaging.broadcast('cluster:job:stop', { ex_id: exId });
            });
        }
    ...
    function _findPort(nodeId) {
        return messaging.send({ to: nodeId, message: 'cluster:node:get_port' });
    }
    ...
    function allocateWorkers(execution, numOfWorkersRequested) {
        ...
        _.forOwn(dispatch.getDispatch(), (workerCount, nodeId) => {
            ...
            results.push(
                messaging.send({ to: nodeId, message: 'cluster:workers:create', payload: requestedWorkersData })
                    .catch(() => {
    ...
    function _createSlicer(job, recoverExecution, errorNodes) {
        ...
        return _findPort(slicerNodeID)
            .then((portObj) => {
                ...
                return messaging.send({ to: slicerNodeID, message: 'cluster:slicer:create', payload: data });
            });
    }
    ...
    function removeWorkers(exId, workerNum) {
        ...
        const results = _.map(nodes, (val, key) => messaging.send({ to: key, message: 'cluster:workers:remove', payload: { workers: val, ex_id: exId } }));
    ...
    function getSlicerStats(exId) {
        return new Promise(((resolve, reject) => {
            ...
            _.each(list, (slicer) => {
                ...
                nodeQueries.push(messaging.send({ to: slicer.node_id, message: 'cluster:slicer:analytics', payload: msg }));
            });
