...
module.exports = function module(context, messaging, executionService) {
        ...
        return _findPort(slicerNodeID)
            .then((portObj) => {
                ...
                return messaging.send({
                    ...
                    .catch((err) => {
                        const error = new VError(err, 'failed to allocate execution_controller to %s', slicerNodeID);
                        logError(logger, error);
                        errorNodes[slicerNodeID] = error;
    ...
    function removeWorkers(exId, workerNum) {
        ...
        return Promise.all(results)
            ...
            .catch((err) => {
                const error = new VError(err, 'failure to release workers from execution %s', exId);
                logError(logger, error);
                return Promise.reject(error);
            });
    }
    ...
    function _notifyNodesWithExecution(exId, messageData, slicerOnly, excludeNode) {
        return new Promise(((resolve, reject) => {
            ...
            if (excludeNode) {
                ...
                const error = new VError('Could not find active execution processes for ex_id:', exId);
            ...
            Promise.map(nodes, (node) => {
                ...
                .catch((err) => {
                    const error = new VError(err, 'failure to notify cluster');
                    logError(logger, error);
                    reject(error);
                });
        }));
