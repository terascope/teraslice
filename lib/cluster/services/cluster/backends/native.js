...
module.exports = function module(context, messaging, executionService) {
    ...
    messaging.register({
        event: 'slicer:execution:finished',
        callback: (msgData) => {
            // remove any pending worker request on completed jobs
            logger.debug(`job for ex_id: ${msgData.ex_id} has finished, removing any from pending queue`, msgData);
