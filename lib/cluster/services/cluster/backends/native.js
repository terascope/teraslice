...
module.exports = function module(context, messaging, executionService) {
    ...
    const {
        node_disconnect_timeout: nodeDisconnectTimeout,
        node_state_interval: nodeStateInterval,
        slicer_allocation_attempts: slicerAllocationAttempts
    ...
    function allocateWorkers(execution, numOfWorkersRequested) {
        const { ex_id: exId, job_id: jobId } = execution;
    ...
    function _createSlicer(job, recoverExecution, errorNodes) {
        ...
        return _findPort(slicerNodeID)
            .then((portObj) => {
                ...
                const { ex_id: exId, job_id: jobId } = job;
