...
module.exports = function(context, cluster_service) {
    ...
    events.on('cluster:job_finished', function(data) {
        logger.info("JobsService: job " + data.job_id + " has completed.");
    ...
    events.on('slicer:slicer_initialized', function(data) {
        logger.info("Job: " + data.job_id + " is now running");
    ...
    events.on('cluster_service:check_job', function(data) {
        ...
        getJob(data.job_id).then(function(job) {
            if (options[job._status]) {
                logger.warn('node ' + data.node_id + ' has disconnected with active workers for job: ' + data.job_id + ' , ' +
                    'enqueuing the workers');
    ...
    function submitJob(job_spec) {
        ...
        return _validateJob(job_spec)
            .then(function(validJob) {
                return _setStatus(validJob, 'pending')
                    .then(function(job) {
    ...
    function getJobs(status, from, size, sort) {
        ...
        // queue may differ.
        return job_store.getJobs(status, from, size, sort);
    }
