...
module.exports = function(context, cluster_service) {
    ...
    events.on('job_failure', function(job_id) {
        getJob(job_id)
            .then(function(job) {
    ...
    //this is for job_completed with errors
    events.on('cluster:error_count', function(data) {
        getJob(data.job_id)
            .then(function(job) {
                _setStatus(job, 'failed', {_errorMessage: data._errorMessage})
            });
    });
                ...
                if (cluster_service.availableWorkers() > 0) {
                    ...
                    _setStatus(job, 'scheduling').then(function() {
                        cluster_service.allocateSlicer(job, recover_job).then(function() {
                            ...
                            .catch(function(err) {
                                logger.error("JobService: Failure during worker allocation - " + err);
                                _setStatus(job, 'failed');
                                throw err;
                            });
                    });
