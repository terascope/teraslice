...
module.exports = function module(context) {
    ...
    function _createExecutionContext(job) {
        return _saveJob(job, 'ex')
            .then(ex => Promise.all([setExecutionStatus(ex.ex_id, 'pending'), _checkModerator(job)])
    ...
    function getExecutionContexts(status, from, size, sort) {
        // TODO should take query instead of status
    ...
    // TODO: try to unify this with getExecutionContexts ? , and better names
    ...
    function getLatestExecution(jobId) {
    ...
    // TODO: look to see if you can refactor this further
    function stopJob(jobId) {
        return getLatestExecution(jobId)
            .then(exId => clusterService.stopJob(exId)
                .then(() => {
    ...
    function pauseJob(jobId) {
        return getLatestExecution(jobId)
            .then(exId => clusterService.pauseJob(exId)
                .then(() => {
                    logger.info(`execution ${exId} has changed status to stopped`);
    ...
    function resumeJob(jobId) {
        return getLatestExecution(jobId)
            .then(exId => clusterService.resumeJob(exId)
                .then(() => {
                    logger.info(`execution ${exId} has changed status to stopped`);
    ...
    function addWorkers(jobId, workerCount) {
        return Promise.all([getLatestExecution(jobId), isExecutionActive(jobId)])
