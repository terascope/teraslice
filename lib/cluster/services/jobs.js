...
module.exports = function module(context) {
    ...
    function submitJob(jobSpec, shouldRun) {
        return _ensureAssets(jobSpec)
            ...
            .catch((err) => {
                ...
                const error = new Error(`Could not submit job, ${errMsg}`);
                logger.error(error);
    ...
    function updateJob(jobId, updatedJob) {
        return _ensureAssets(updatedJob)
            ...
            .catch((err) => {
                const errMsg = parseError(err);
                const error = new Error(`could not updateJob ${errMsg}`);
                logger.error(error);
                return Promise.reject(error);
            });
    }
        ...
        return _getActiveExecution(jobId, true)
            .then((execution) => {
                ...
                return getJob(jobId)
                    .then((jobConfig) => {
                        if (!jobConfig) {
                            return Promise.reject(new Error(`no job for job_id: ${jobId} could be found`));
                        }
            ...
            .catch((err) => {
                const error = new Error(`could not start job, error: ${parseError(err)}`);
                logger.error(error);
                return Promise.reject(error);
            });
    }
    ...
    function recoverJob(jobId) {
        ...
        if (!jobId) {
            return Promise.reject(new Error('Recover Job requires jobId'));
        }
        ...
        return executionService.searchExecutionContexts(query, null, 1, '_created:desc')
            .then((ex) => {
                if (!allowZeroResults && ex.length === 0) {
                    return Promise.reject(new Error(`no execution context was found for job_id: ${jobId}`));
                }
                return ex[0];
            });
    }
    ...
    function _validateJob(jobSpec) {
        return new Promise(((resolve, reject) => {
            ...
            try {
                const validJob = jobValidator.validate(jobSpec);
                resolve(validJob);
            } catch (ev) {
                reject(new Error(`Error validating job: ${ev}`));
            }
        }));
    }
            ...
            if (!jobAssets) {
                ...
                executionService.verifyAssets(jobAssets)
                    .then((assetMsg) => {
                        ...
                        if (assetMsg.error) {
                            const error = new Error(assetMsg.error);
                            reject(error);
                            return;
                        }
                        if (assetIds === 0) {
                            const error = new Error(`no asset id's were found for assets: ${JSON.stringify(jobAssets)}`);
                            error.code = 404;
                            reject(error);
                            return;
                        }
                        ...
                        if (jobAssets.length !== assetIds.length) {
                            const error = new Error(`job specified ${jobAssets.length} assets: ${jobAssets.assets} but only ${assetIds.length} where found, assets: ${assetIds}`);
                            reject(error);
                            return;
                        }
                    ...
                    .catch(err => reject(new Error(`could not parse assets for their id's, error: ${parseError(err)}`)));
            }
        });
    }
