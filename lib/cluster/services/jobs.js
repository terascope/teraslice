...
module.exports = function(context, cluster_service) {
    ...
    function _allocateJobs() {
        function allocateJob() {
            ...
            //TODO noop state if a job is being already being allocated
            if (pendingJobsQueue.size() > 0) {
                //TODO make it greater than 2
                if (cluster_service.availableWorkers() > 0) {
                    ...
                    _setStatus(job, 'scheduling').then(function() {
                        //TODO allocateSlicer and Workers are not transactional
                        cluster_service.allocateSlicer(job, recover_job).then(function() {
                            return cluster_service.allocateWorkers(job, job.workers)
                                .then(function() {
                                    ...
                                    //TODO this is duplicate, its already on a timer
                                    allocateJob();
                                })
                        })
                            .catch(function(err) {
