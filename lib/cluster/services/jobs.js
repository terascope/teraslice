...
module.exports = function(context) {
    ...
    var messaging = messageModule(context, logger, events);
    var api;

//need a way to wait till api is up, but i need to set this up fast
    messaging.register('service', function(ipcMessage) {
        var response = {service: ipcMessage._from, _msgID: ipcMessage._msgID, message: 'service'};

        Promise.resolve(api[ipcMessage.method].apply(null, ipcMessage.args))
    ...
    Promise.all([require('../storage/jobs')(context, 'job'), require('../storage/jobs')(context, 'ex')])
        ...
        .then(function() {
            ...
            function pauseJob(connectionList) {
                let jobList = [];
            ...
            function jobsResume(connectionList) {
            ...
            function checkModerator(job) {
                ...
                return messaging.notifyService('cluster_service', {
                    method: 'checkModerator',
                    args: [connectionList]
                })
            ...
            function ensureAssets(job_spec) {
                return new Promise(function(resolve, reject) {
                    if (!job_spec.assets) {
                        ...
                        var msgObj = {message: 'jobs_service:verify_assets', assets: job_spec.assets, _msgID: id};
                        ...
                        events.once(id, function(msg) {
                            ...
                            if (msg.assets.length !== job_spec.assets.length) {
                                reject(`job specified ${job_spec.assets.length} assets: ${job_spec.assets} but only ${msg.assets.length} where found, assets: ${msg.assets}`);
                                return
                            }
            ...
            function jobAllocator() {
                ...
                return function allocator() {
                    //TODO might need a guard to prevent multiple calls
                    messaging.notifyService('cluster_service', {
                        method: 'availableWorkers',
                        args: []
                    })
                        .then(function(ipcResponse) {
                            var availableWorkers = ipcResponse.data;
                            if (!isJobBeingAllocated && pendingExecutionQueue.size() > 0 && availableWorkers >= 2) {
                                ...
                                _setStatus(executionContext, 'scheduling')
                                    .then(function() {
                                        messaging.notifyService('cluster_service', {
                                            method: 'allocateSlicer',
                                            args: [executionContext, recover_execution]
                                        })
                                            .then(function(results) {
                                                executionContext = JSON.parse(results.data.job);
                                                return _setStatus(executionContext, 'initializing');
                                            })
                                            .then(function() {
                                                messaging.notifyService('cluster_service', {
                                                    method: 'allocateWorkers',
                                                    args: [executionContext, executionContext.workers]
                                                })
                                                    .then(function() {
            ...
            function _notifyCluster(ex_id, notice) {
                ...
                return new Promise(function(resolve, reject) {
                    ...
                    return messaging.notifyService('cluster_service', {
                        method: 'findNodesForJob',
                        args: [ex_id, slicerOnly]
                    })
                        .then(function(ipcResponse) {
                            var nodes = ipcResponse.data;

                            nodes.forEach(function(node) {
                                var messageNode = messaging.notifyService('cluster_service', {
                                    method: 'notifyNode',
                                    args: [node.node_id, message, {ex_id: ex_id}]
                                });
            ...
            api = {
                ...
                slicerFailure: slicer_failure,
                processingError: processingError,
                slicerInitialized: slicerInitialized,
                jobUpdate: jobUpdate,
