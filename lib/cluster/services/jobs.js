...
module.exports = function module(context, clusterService) {
    ...
    // FIXME: this should reside in cluster service, just call in to get ex
    ...
    function _jobAllocator() {
        ...
        return function allocator() {
            // FIXME: clusterService.readyForAllocation
    ...
    function _notifyCluster(exId, notice, excludeNode) {
        ...
        // TODO: make a cluster fn encapsulating below => notifyNodesForEx(exId, slicerOnly)
        return new Promise(((resolve, reject) => {
            const requests = [];
    ...
    function shutdown() {
        ...
        return _exSearch(query)
            .map((job) => {
                // FIXME: only applies to native clustering
    ...
    // FIXME: refactor restartExecution to be used by recoverJob
    ...
    const api = {
        ...
        resumeJob,
        recoverJob,
        restartExecution, // will be removed
        shutdown
    };
