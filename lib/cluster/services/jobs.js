...
//TODO make sure names correctly reflect jx vs job
...
module.exports = function(context, cluster_service) {
    ...
    events.on('cluster:job_finished', function(data) {
        logger.info(`JobsService: job ${data.jx_id} has completed`);
    ...
    events.on('slicer:processing_error', function(data) {
        logger.error(`JobsService: job ${data.jx_id} has had at least one slice failure and will be marked 'failed'`);
        _setStatus(data, 'failing')
    });

    events.on('slicer:slicer_initialized', function(data) {
        logger.info(`Job: ${data.jx_id} is now running`);
    ...
    events.on('cluster_service:check_job', function(data) {
        ...
        getJob(data.jx_id).then(function(job) {
            if (options[job._status]) {
                logger.warn(`node ${data.node_id} has disconnected with active workers for job: ${data.jx_id} , enqueuing the workers`);
        ...
        return _validateJob(job_spec)
            .then(function(validJob) {
                if (!shouldRun) {
                    ...
                    var cloneValidJob = _.clone(validJob);
                    return Promise.all([saveJob(validJob, 'job'), saveJob(cloneValidJob, 'jx')])
                        .spread(function(job, jx) {
                            jx.job_id = job.job_id;

                            return _setStatus(jx, 'pending')

                        }).then(function(jx) {
                            pendingJobsQueue.enqueue(jx);
                            return {job_id: jx.job_id, jx_id: jx.jx_id};
                        });
                }
            });
    }
    ...
    function getJobs(status, from, size, sort) {
        ...
        // queue may differ.
        return jx_store.getJobs(status, from, size, sort);
    }
    ...
    function _signalJobStateChange(jx_id, notice, state) {
        return _notifyCluster(jx_id, notice)
            .then(function() {
                return jx_store.get(jx_id)
                    .then(function(job_spec) {
                        logger.info(`JobsService: job ${jx_id} has changed to state '${state}`);
    ...
    function _notifyCluster(jx_id, notice) {
        ...
        return new Promise(function(resolve, reject) {
            var requests = [];
            var nodes = cluster_service.findNodesForJob(jx_id, slicerOnly);
    ...
    function _setStatus(job_spec, status, metaData) {
        if (VALID_STATUS.indexOf(status) != -1) {
            if (job_spec.jx_id) {
                ...
                return jx_store.update(job_spec.jx_id, status)
                    .then(function() {
                        return job_spec;
                    });
            }
    ...
    function shutdown() {
        ...
        return Promise.all([job_store.shutdown(), jx_store.shutdown()]);
    }
    ...
    return Promise.all([require('../storage/jobs')(context, 'job'), require('../storage/jobs')(context, 'jx')])
        .spread(function(job, jx) {
