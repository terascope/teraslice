...
module.exports = function module(context) {
    ...
    function updateJob(jobId, updatedJob) {
        return _ensureAssets(updatedJob)
            ...
            .catch((err) => {
                const error = new VError(err, 'Could not update job');
    ...
    function startJob(jobId) {
        return _getActiveExecution(jobId, true)
            ...
            .catch((err) => {
                const error = new VError(err, 'could not start job');
                logError(logger, error);
                return Promise.reject(error);
            });
    }
    ...
    function _validateJob(jobSpec) {
        return new Promise(((resolve, reject) => {
            ...
            try {
                ...
                const error = new VError('failure to validate job');
    ...
    function shutdown() {
        return jobStore.shutdown()
            .catch((err) => {
                ...
                }, 'failure while shutting down job stores');
                logError(logger, error);
        ...
        return new Promise((resolve, reject) => {
            if (!jobAssets) {
                ...
                executionService.verifyAssets(jobAssets)
                    ...
                    .catch((err) => {
                        const error = new VError(err, 'failure to parse asset id\'s');
                        reject(error);
                    });
            }
        });
