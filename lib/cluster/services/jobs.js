...
module.exports = function module(context) {
    ...
    function submitJob(jobSpec, shouldRun) {
        return executionService.ensureAssets(jobSpec)
            .then(parsedAssetJob => _validateJob(parsedAssetJob))
            .then((validJob) => {
                console.log('i got the valid job');
                ...
                return _createJob(validJob)
                    .then((job) => {
                        ...
                        console.log('i saved job now im creating execution context');
    ...
    function startJob(jobId) {
        return executionService.isExecutionActive(jobId)
            .then((isAlreadyRunning) => {
                ...
                return getJob(jobId)
                    .then(jobSpec => executionService.ensureAssets(jobSpec))
    ...
    function getActiveExecution(jobId) {
    ...
    function getActiveExecutionId(jobId) {
        return getActiveExecution(jobId)
            .then(ex => ex.ex_id);
    }
        ...
        return getLatestExecutionId(jobId)
            .then(exId => executionService.stopExecution(exId)
                .then(() => {
                    ...
                    return executionService.setExecutionStatus(exId, STATE_MAPPING.stop);
                })
        ...
        return getLatestExecutionId(jobId)
            .then(exId => executionService.pauseExecution(exId)
                .then(() => {
                    ...
                    return executionService.setExecutionStatus(exId, STATE_MAPPING.pause);
                })
        ...
        return getLatestExecutionId(jobId)
            .then(exId => executionService.resumeExecution(exId)
                .then(() => {
                    ...
                    return executionService.setExecutionStatus(exId, STATE_MAPPING.resume);
                })
    ...
    function setWorkers(jobId, workerCount) {
        return Promise.all([getLatestExecutionId(jobId), isExecutionActive(jobId)])
            .spread((exId, isActive) => {
                if (isActive) {
                    const totalWorker = executionService.findWorkersByExecutionID(exId).length;
                    if (totalWorker > workerCount) {
                        const removedWorkersCount = totalWorker - workerCount;
                        return executionService.removeWorkers(exId, removedWorkersCount);
                    }
                    if (totalWorker < workerCount) {
                        const addWorkersCount = workerCount - totalWorker;
                        return executionService.getExecutionContext(exId)
                            .then(execution => executionService.addWorkers(
                                execution,
                                addWorkersCount
                            ));
                    }
