...
module.exports = function module(context) {
    ...
    function _createExecutionContext(job) {
        return _saveJob(job, 'ex')
            .then(ex => _setExecutionStatus(ex.ex_id, 'pending')
                .then(() => {
    ...
    function _setExecutionStatus(exId, status, metaData) {
    ...
    function _jobAllocator() {
        ...
        return function allocator() {
            ...
            if (!allocatingJob && pendingQueueSize > 0 && clusterService.readyForAllocation()) {
                ...
                _setExecutionStatus(execution.ex_id, 'scheduling')
                    .then(() => {
                        clusterService.allocateSlicer(execution, recoverExecution)
                            .then(() => _setExecutionStatus(execution.ex_id, 'initializing', {
                            ...
                            .catch((err) => {
                                ...
                                allocatingJob = false;
                                _setExecutionStatus(execution.ex_id, 'failed');
                                allocator();
                            });
                    });
            }
        ...
        return getLatestExecutionId(jobId)
            .then(exId => clusterService.stopExecution(exId)
                .then(() => {
                    ...
                    return _setExecutionStatus(exId, STATE_MAPPING.stop);
                })
        ...
        return getLatestExecutionId(jobId)
            .then(exId => clusterService.pauseExecution(exId)
                .then(() => {
                    ...
                    return _setExecutionStatus(exId, STATE_MAPPING.pause);
                })
        ...
        return getLatestExecutionId(jobId)
            .then(exId => clusterService.resumeExecution(exId)
                .then(() => {
                    ...
                    return _setExecutionStatus(exId, STATE_MAPPING.resume);
                })
        ...
        return _notifyCluster(exId, notice, excludeNode)
            .then(() => exStore.get(exId)
                .then((jobSpec) => {
                    ...
                    return _setExecutionStatus(jobSpec.ex_id, state);
                })
                .then(() => state))
            .catch((err) => {
                const errMsg = parseError(err);
    ...
    function _isNative() {
        return clusterService.getClusterType === 'native';
    }
