...
module.exports = function(context, cluster_service) {
    var logger = context.foundation.makeLogger('jobs', 'jobs', {service: 'jobs'});
    ...
    events.on('cluster:job_failure', function(data) {
        ...
        logger.error(`JobsService: job ${data.ex_id} has failed to complete`);
    ...
    events.on('slicer:job_update', function(updateSpec) {
                ...
                _setStatus(executionContext, 'scheduling')
                    .then(function() {
                        cluster_service.allocateSlicer(executionContext, recover_execution).then(function() {
                            ...
                            .catch(function(err) {
                                var errMsg = elasticError(err);
                                logger.error(`JobsService: Failure during worker allocation - ${errMsg}`);
                                isJobBeingAllocated = false;
    ...
    function _initialize() {
        ...
        return getExecutionContexts('running').each(function(job) {
            ...
            .error(function(err) {
                ...
                var errMsg = elasticError(err);
                logger.error(`Error initializing job service, `, errMsg);
