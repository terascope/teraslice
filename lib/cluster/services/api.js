...
module.exports = function module(context, app) {
    ...
    // TODO: verify if error messages can have the errors in it
    ...
    app.post('/ex/:ex_id/_pause', (req, res) => {
        ...
        // FIXME: this method does not exist => pauseExecution
        clusterService.pause(exId)
    ...
    app.get('/ex/:ex_id/slicer', (req, res) => {
        ...
        _slicerStats(exId)
            ...
            .catch((err) => {
                const errMsg = parseError(err);
                logger.error(`could not get slicer statistics, error: ${errMsg}`);
                // FIXME: can't be sure it will return with code
                sendError(res, err.code, errMsg);
            });
    });
