...
module.exports = function(context, app, cluster_service, jobs_service) {
    ...
    app.post('/jobs/:id/_workers', function(req, res) {
    ...
    app.get('/txt/workers', function(req, res) {
        var defaults = ['assignment', 'job_id','node_id', 'pid'];
    ...
    app.get('/txt/jobs', function(req, res) {
        var defaults = ['job_id', 'name', 'lifecycle', 'slicers', 'workers', '_created', '_updated', '_status'];
        var size = 10000;
    ...
    function changeWorkers(req, res) {
        ...
        if (!msg || isNaN(workerNum) || workerNum <= 0) {
            ...
            if (msg === 'total') {
                var totalWorker = cluster_service.findWorkersByJobID(job_id).length;
            ...
            jobs_service.getJob(job_id)
                .then(function(job) {
                    if (jobState[job._status]) {
                        if (msg === 'add') {
                            //TODO fix this, get rid of getJobInfo
