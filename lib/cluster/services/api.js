...
module.exports = function(context, app, cluster_service, jobs_service) {
    ...
    app.post('/jobs/:id/_addWorkers', function(req, res) {
        changeWorkers(req, res, 'add');
    });

    app.post('/jobs/:id/_removeWorkers', function(req, res) {
        changeWorkers(req, res, 'remove');
    });
    ...
    function changeWorkers(req, res, msg) {
        var job_id = req.params.id;
        if (!req.query || !req.query.workers) {
            sendError(res, 400, 'Must provide a workers parameter in request');
            return
        }

        var workerNum = Number(req.query.workers);
        var options = {running: true, failing: true, paused: true};

        if (isNaN(workerNum) || workerNum <= 0) {
            sendError(res, 400, 'Must provide a worker parameter that is a number and greater than zero');
        }
        else {
            jobs_service.getJob(job_id)
                .then(function(job) {
                    if (options[job._status]) {
                        if (msg === 'add') {
                            var jobData = _.assign(job, cluster_service.getJobInfo(job_id));
                            var workerRequest = {
                                job_id: job_id,
                                id: job_id,
                                workers: workerNum,
                            ...
                            res.status(200).send(workerNum + ' workers have been enqueued for job: ' + job_id);
                            return
                        }
                        if (msg === 'remove') {
                            cluster_service.removeWorkers(res, job_id, workerNum);
                        }
                    }
                    else {
