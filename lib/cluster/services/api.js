...
module.exports = function(context) {
    var logger = context.foundation.makeLogger({module: 'api_service'});
    ...
    app.get('/cluster/slicers', function(req, res) {
        ...
        slicerStats()
            .then(function(results) {
                res.status(200).send(results.map(msg => msg.data));
            })
            .catch(function(err) {
                    ...
                    if (jobState[ex._status]) {
                        ...
                        if (msg === 'remove') {
                            messaging.notifyService('cluster_service', {
                                ...
                                args: [ex_id, workerNum]
                            })
                                .then(function(ipcResponse) {
                                    var resMsg = ipcResponse.data;
                                    if (resMsg && resMsg.error) {
                                        logger.error(`Error removing workers to queue, error: ${resMsg.error}`);
                                        sendError(res, 500, resMsg.error);
                                    }
                                    else {
