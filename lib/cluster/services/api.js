...
module.exports = function module(context, app) {
    ...
    const { messaging, services: { execution: executionService, jobs: jobsService } } = context;
    ...
    app.post('/jobs/:job_id/_stop', (req, res) => {
        const { params: { job_id: jobId }, query: { timeout } } = req;
    ...
    app.get('/jobs/:job_id/errors/', (req, res) => {
        const { query: { from, size = 10000 }, params: { job_id: jobId } } = req;
    ...
    app.get('/jobs/:job_id/errors/:ex_id', (req, res) => {
        const { query: { from, size = 10000 }, params: { job_id: jobId, ex_id: exId } } = req;
    ...
    app.get('/ex', (req, res) => {
        const { status, from, size, sort } = req.query;
        const handleApiError = handleError(res, logger, 500, 'Could not retrieve list of execution contexts');
    ...
    app.post('/ex/:ex_id/_stop', (req, res) => {
        const { query: { timeout }, params: { ex_id: exId } } = req.params;
    ...
    app.post('/ex/:ex_id/_workers', (req, res) => {
        const { query, params: { ex_id: exId } } = req;
