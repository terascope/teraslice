...
module.exports = function(context, app, services) {
    ...
    var assetsUrl = `http://192.168.1.17:${process.env.assets_port}`;
    ...
    app.post('/assets', function(req, res) {
        req.pipe(request.post(`${assetsUrl}/assets`).on('response', function(assetsResponse) {
            assetsResponse.pipe(res)
        }))
    });

    app.delete('/assets/:asset_id', function(req, res) {
        cluster_service.broadcast('assets:delete', req.params.asset_id);
        req.pipe(request.delete(`${assetsUrl}/assets/${req.params.asset_id}`).on('response', function(assetsResponse) {
            assetsResponse.pipe(res)
        }))
    });
