...
module.exports = function(context, server) {
    ...
    messaging.register('node:online', function(data) {
        ...
        //this also decorates the socket with node_id
        messaging.isolateConnection.call(this, 'node_id', data.node_id);
    ...
    messaging.register('node:disconnect', function() {
        var node = messaging.getDecoration.call(this, 'node_id');

        if (node && cluster_state[node]) {
            if (cluster_state[node].active.length === 0) {
                logger.info(`node ${node} has disconnected`);
                delete cluster_state[node];
            }
            else {
                cluster_state[node].state = 'disconnected';

                var timer = setTimeout(function() {
                    cleanUpNode(node);
                }, 5000);

                droppedNodes[node] = timer;
            }
        }
        else {
            logger.error(`cluster got a node_master disconnect event, but no node_id or state was found. might be a network issue, node_id: ${node}`, cluster_state[node])
        }
    });
