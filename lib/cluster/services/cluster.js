...
module.exports = function(context, server) {
    ...
    io.on('connection', function(socket) {
        ...
        socket.on('slicer:job_finished', function(data) {
            ...
            pendingWorkerRequests.remove(data.jx_id);
            
            io.emit('cluster_service:stop_job', data);
            //if errors in slices, emit error event
            if (data.errorCount > 0) {
                var message = `job: ${data.jx_id} had ${data.errorCount} slice failures during processing`;
                data.error = message;
        ...
        socket.on('slicer:failed_recovery', function(data) {
            ...
            pendingWorkerRequests.remove(data.jx_id);
            io.emit('cluster_service:stop_job', {jx_id: data.jx_id});
            event.emit('cluster:job_failure', data);
        });
...
//review this
    ...
    function allocateWorker(job) {
        ...
        var data = {job: jobStr, jx_id: jx_id, workers: 1, node_id: workerNodeID, assignment: 'worker'};
    ...
    function allocateWorkers(job, numOfWorkersRequested) {
        ...
        var workerData = {job: jobStr, id: jx_id, jx_id: jx_id, workers: 1, assignment: 'worker'};
    ...
    function findAllByJobID(jx_id) {
        var allByID = function(worker) {
            return worker.jx_id === jx_id;
        };

        return iterate_state(allByID)
    }

    function findWorkersByJobID(jx_id) {
        var workersByJobID = function(worker) {
            return worker.assignment === 'worker' && worker.jx_id === jx_id;
        };

        return iterate_state(workersByJobID)
    }

    function findSlicersByJobID(jx_id) {
        var slicerByJobID = function(worker) {
            return worker.assignment === 'slicer' && worker.jx_id === jx_id;
        };

        return iterate_state(slicerByJobID)
    }

    function removeWorkers(res, jx_id, workerNum) {
        var dispatch = makeDispatch();
        var workers = findWorkersByJobID(jx_id);
        ...
        var results = _.map(nodes, function(val, key) {
            return notifyNode(key, 'cluster_service:remove_workers', {workers: val, jx_id: jx_id})
        });

        Promise.all(results)
            .then(function() {
                res.status(200).send(`${workerNum} workers have been released from job: ${jx_id}`)
            })
            .catch(function(err) {
                res.status(500).json({
