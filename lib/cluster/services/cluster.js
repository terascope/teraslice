...
module.exports = function(context, server) {
    var messaging = messageModule(context, server);
    ...
    event.on('cluster:error:terminal', terminalShutdown);
    ...
    messaging.register('worker:message', function(nodeMessage) {
        if (nodeMessage.message === 'shutdown') {
            event.emit('cluster_master:shutdown')
        }
    });

    messaging.register('node:online', function(data) {
        logger.info(`node ${data.node_id} has connected`);
        messaging.joinRoom(data.node_id);
        messaging.decorateConnection('node_id', data.node_id);
    ...
    messaging.register('slicer:failed_recovery', function(data) {
    ...
    messaging.register('job:terminal_error', function(data) {
        pendingWorkerRequests.remove(data.ex_id);
        messaging.send('cluster:job:stop', {ex_id: data.ex_id});
        event.emit('cluster:job_failure', data);
    });

    messaging.register('slicer:processing_error', function(data) {
        event.emit('slicer:processing_error', data)
    });

    messaging.register('slicer:slicer_initialized', function(data) {
        event.emit('slicer:slicer_initialized', data)
    });

    messaging.register('slicer:job_update', function(ex_Update) {
        event.emit('slicer:job_update', ex_Update);
    });

    messaging.register('node:disconnect', function() {
        var node = messaging.getDecoration('node_id');
    ...
    messaging.register('cluster:connection:error', function(ev) {
        logger.error(ev)
    });
    ...
    function cleanUpNode(node_id) {
        ...
        if (node.hasSlicer) {
            _.forIn(node.slicerJobID, function(value) {
                ...
                event.emit('cluster:job_failure', data);
                messaging.send('cluster:job:stop', {ex_id: value});
            });
        }
