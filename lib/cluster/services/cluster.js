    ...
    io.on('connection', function(socket) {
        ...
        socket.on('slicer:job_finished', function(data) {
            ...
            if (data.errorCount > 0) {
                var message = 'job: ' + data.job_id + ' has completed with errors on ' + data.errorCount + ' slices.';
                data._errorMessage = message;

                logger.warn(message);
                event.emit('cluster:error_count', data)
            }
            else {
        ...
        socket.on('slicer:failed_recovery', function(data) {
            ...
            pendingWorkerRequests.remove(data.job_id);
            io.emit('cluster_service:stop_job', {job_id: data.job_id});
            event.emit('job_failure', data.job_id)

        });

        //error on slicer creation
