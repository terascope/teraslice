...
module.exports = function(context) {
    ...
    var logger = context.foundation.makeLogger({module: 'cluster_service'});
    logger.info("Initializing");

    var messaging = messageModule(context, logger, events);

    var server = app.listen(clusterConfig.node_port);
    ...
    //TODO need proper shutdown of service
...
//TODO review this
    ...
    function removeWorkers(ex_id, workerNum) {
        var dispatch = makeDispatch();
        ...
        return Promise.all(results)
            .then(function() {
                return {response: `${workerNum} workers have been released from job: ${ex_id}`}
            })
            .catch(function(err) {
