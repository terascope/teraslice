...
module.exports = function(context, server) {
    ...
    //TODO refactor this to use messageTransaction
...
// TODO: this is a placeholder function with a fire an forget impl.
// Needs to be replaced with an acknowledged update.
        //send message
    ...
    //TODO make this reusable with other inquiries
    function messageTransaction(node_id, msg, msgData) {
        return new Promise(function(resolve, reject) {
            //set up the listener
            event.on('worker online', function(data) {

                if (data.assignment === msgData.assignment && data.job_id === msgData.job_id && data.node_id === msgData.node_id) {
                    //remove listener to prevent memory leaks
                    event.removeAllListeners('worker online');
                    resolve(data)
                }
            });
            //send message
    ...
    function allocateWorker(job) {
        ...
        return messageTransaction(workerNodeID, 'create workers', data);
    }
    ...
    function allocateSlicer(job) {
        ...
        var data = {job: jobStr, job_id: job_id, workers: 1, node_id: slicerNodeID, assignment: 'slicer'};


        //TODO figure out how to change the slicer_port, slicer_hostname and jobStr if the slicer creation fails
        return messageTransaction(slicerNodeID, 'create slicer', data);
    }
