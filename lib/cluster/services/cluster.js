...
module.exports = function(context, server) {
    var network = messaging({type: 'network', isClient: false, server: server, context:context});
    ...
    network.register('node_master:node_online', function(data) {
        logger.info(`node ${data.node_id} has connected`);
        network.joinRoom(data.node_id);
        network.decorateConnection('node_id', data.node_id);
    ...
    network.register('node_master:node_state', function(data) {
        cluster_state[data.node_id] = data;
        event.emit('cluster:available_workers')
    });

    network.register('node_master:message_processed', function(data) {
        //emitting the unique msg id to allow easier listener cleanup
        event.emit(data._msgID, data);
    });

    network.register('node_master:over_allocated_workers', function(requestData) {
        pendingWorkerRequests.enqueue(requestData);
    });

    network.register('slicer:job_finished', function(data) {
        //remove any pending worker request on completed jobs
        pendingWorkerRequests.remove(data.ex_id);

        network.send('cluster_service:stop_job', data);
    ...
    network.register('slicer:failed_recovery', function(data) {
        //remove any pending worker request
        pendingWorkerRequests.remove(data.ex_id);
        network.send('cluster_service:stop_job', {ex_id: data.ex_id});
        event.emit('cluster:job_failure', data);
    });

    network.register('job:terminal_error', function(data) {
        pendingWorkerRequests.remove(data.ex_id);
        network.send('cluster_service:stop_job', {ex_id: data.ex_id});
        event.emit('cluster:job_failure', data);
    });

    network.register('slicer:processing_error', function(data) {
        event.emit('slicer:processing_error', data)
    });

    network.register('slicer:slicer_initialized', function(data) {
        event.emit('slicer:slicer_initialized', data)
    });

    network.register('slicer:job_update', function(ex_Update) {
        event.emit('slicer:job_update', ex_Update);
    });

    network.register('disconnect', function() {
        var node = network.getDecoration('node_id');
        if (cluster_state[node].active.length === 0) {
            logger.info(`node ${node} has disconnected`);
            delete cluster_state[node];
        }
        else {
    ...
    network.register('error', function(ev) {
        logger.error(ev)
    });
    ...
    setInterval(function() {
        network.send('cluster_service:get_node_state')
    }, 5000);
    ...
    function cleanUpNode(node_id) {
        ...
        if (node.hasSlicer) {
            _.forIn(node.slicerJobID, function(value) {
                logger.warn(`node ${node_id} has disconnected with active slicer for job: ${value}, marking as failed and shutting down job`);

                var data = {ex_id: value, error: 'node_master where slicer resided has disconnected'};
                pendingWorkerRequests.remove(value);
                event.emit('cluster:job_failure', data);
                network.send('cluster_service:stop_job', {ex_id: value});
            });
        }
    ...
    function notifyNode(node_id, msg, msgData) {
        return new Promise(function(resolve, reject) {
            ...
            //send message
            network.send(node_id, msg, msgData);
    ...
    function removeWorkers(res, ex_id, workerNum) {
        ...
        var results = _.map(nodes, function(val, key) {
            return notifyNode(key, 'cluster_service:remove_workers', {workers: val, ex_id: ex_id})
        });
