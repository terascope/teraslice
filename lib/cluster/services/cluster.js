...
module.exports = function(context, server) {
    ...
    var logger = context.foundation.makeLogger('cluster', 'cluster', {service: 'cluster'});
    ...
    io.on('connection', function(socket) {
        ...
        socket.on('slicer:failed_recovery', function(data) {
            ...
            pendingWorkerRequests.remove(data.ex_id);
            io.emit('cluster_service:stop_job', {ex_id: data.ex_id});
            event.emit('cluster:job_failure', data);
        });

        socket.on('job:terminal_error', function(data) {
            pendingWorkerRequests.remove(data.ex_id);
            io.emit('cluster_service:stop_job', {ex_id: data.ex_id});
            event.emit('cluster:job_failure', data);
        });
        ...
        socket.on('slicer:job_update', function(ex_Update) {
    ...
    function cleanUpNode(node_id) {
        ...
        if (node.hasSlicer) {
            _.forIn(node.slicerJobID, function(value) {
                ...
                event.emit('cluster:job_failure', data);
                io.emit('cluster_service:stop_job', {ex_id: value});
            });
        }
