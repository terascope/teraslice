...
function makeConfigurations(context) {
    ...
    var options = {
        ...
        jobs_service: {networkClient: false, ipcClient: true},
        api_service: {networkClient: false, ipcClient: true},
        cluster_service: {networkClient: false, ipcClient: true},
    ...
    if (config.clients.networkClient) {
        if (env.assignment === 'node_master' || env.assignment === 'moderator' || env.assignment === 'assets_service') {
            ...
            port = context.sysconfig.teraslice.node_port;
        }
...
module.exports = function messaging(context, logger, events, childHookFn) {
    var processContext;
    var io;
   ...
   /* //TODO review this
   register('service', function(data) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            events.on(msgData._msgID, function(nodeMasterData) {
                ...
                if (nodeMasterData.error) {
                    reject(`Error: ${nodeMasterData.error} occured `)
                }
                else {
                    logger.trace(`cluster_master completed message transaction with  msgData:`, msgData);
                    resolve(nodeMasterData)
                }
            });
            //send message
            logger.debug(`cluster_master communicating with : ${service}`);
            logger.trace(`cluster_master communicating with : ${service}, msgData:`, msgData);
            msgData.service = service;
            msgData.message = 'service';

            process.send(msgData);

            //reject if timeout has been reached
            setTimeout(function() {
                //remove listener to prevent memory leaks
                events.removeAllListeners(msgData._msgID);
                reject(`Error communicating with , Could not send data: ${JSON.stringify(msgData)}`)
            }, configTimeout)
        })
    }
    ...
    if (config.clients.ipcClient) {
        process.on('message', function(ipcMessage) {
            ...
            if(ipcMessage.isResponse){
                events.emit(ipcMessage._msgID, ipcMessage);
            }
            else {
        ...
        processContext.on('online', function(worker) {
            ...
            processContext.workers[worker.id].on('message', function(ipcMessage) {
                if (networkMapping[ipcMessage.message]) {
                    ...
                    if(ipcMessage.message === 'service'){
                        messageWorkers(context, ipcMessage, ipcMessage, function(worker) {
                            return worker.assignment === ipcMessage.service;
                        });
                    }
                    else {
