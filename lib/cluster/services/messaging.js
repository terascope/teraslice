...
const clusterMasterMessages = {
    ...
    network: {
        ...
        'slicer:execution:finished': 'slicer:execution:finished',
...
const routing = {
    cluster_master: { slicer: 'network', node_master: 'network', assets_loader: 'ipc' },
    node_master: { cluster_master: 'network', slicer: 'ipc', worker: 'ipc', assets_loader: 'ipc', execution: 'ipc' },
    slicer: { worker: 'network', cluster_master: 'ipc', node_master: 'ipc' },
    worker: { slicer: 'network', cluster_master: 'ipc', node_master: 'ipc' },
...
module.exports = function messaging(context, logger, childHookFn) {
    ...
    function handleResponse(msgResponse) {
        ...
        if (msgResponse.__source === config.assignment) {
            logger.debug(`node message ${msgResponse._msgID} has been processed`);
            // we are in the right spot, emit to complete the promise from send
            events.emit(msgResponse._msgID, msgResponse);
        } else {
            // Forward the message on
            const messageType = determinePathForMessage(msgResponse);
            if (messageType === 'network') {
                ...
                if (self === 'worker' || self === 'node_master') {
                    broadcast('messaging:response', msgResponse);
                } else {
                    io.sockets.in(msgResponse.address).emit('messaging:response', msgResponse);
                }
            } else {
                // TODO: this may need more specific process sending
                processContext.send(msgResponse);
            }
        }
    }
    ...
    function respond(incoming, outgoing) {
        ...
        if (incoming._msgID) {
            outgoingResponse._msgID = incoming._msgID;
        }
    ...
    function register(key, fnArg, arg3) {
        ...
        if (selfMessages.ipc[key]) {
            const realKey = selfMessages.ipc[key];
    ...
    function send(messageSent) {
        ...
        if (!needsReply) {
            ...
            if (messageType === 'network') {
                // worker and node_master communicate through broadcast to slicer/cluster_master
                if (self === 'worker' || self === 'node_master') {
                    broadcast(messageSent.message, messageSent);
