...
module.exports = function messaging(context, logger) {
    ...
    function registerFns(socket) {
        for (let key in functionMapping) {
            ...
            if (func.__wrapper) {
                ...
                logger.trace(`setting listener key ${key} to action ${func}`);
                socket.on(key, func)
            }
        }
    }
        ...
        if (config.clients.networkClient) {
            ...
            if (server) {
                ...
                io.on('connection', function(socket) {
                    logger.trace(`a node_master has connected`);
                    registerFns(socket);
                });
            }
            else {
                ...
                io.on('connection', function(socket) {
                    logger.trace(`a worker has connected`);
                    registerFns(socket);
                });
                io.listen(port);
    ...
    if (!config.clients.ipcClient) {
        processContext.on('online', function(worker) {
            logger.trace('worker has come online');
