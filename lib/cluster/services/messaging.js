...
module.exports = function messaging(context, logger) {
    ...
    // the cluster master usually communicates to node masters through network, even its parent
    // node, however there are certain circumstances that communication must by through ipc to its
    // parent node such as a SIGTERM/SIGINT or a startup issue of cluster master
    function clusterMasterBypass(messageSent) {
        if (self === 'cluster_master' && !messageSent.address) {
            const message = messageSent.message;
            if (clusterMasterMessages.ipc[message] || message === 'cluster:error:terminal') {
                return true;
            }
        }
        return false;
    ...
    function _determinePathForMessage(messageSent) {
        ...
        if (clusterMasterBypass(messageSent)) {
            destinationType = 'ipc';
        }
