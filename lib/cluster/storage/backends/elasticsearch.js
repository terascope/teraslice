...
module.exports = function(context, index_name, record_type, id_field, bulk_size) {
    ...
    function search(query, from, size, sort) {
        logger.debug(`elasticsearch search call, sort: ${sort}, from: ${from}, size: ${size}, query: `, query);
        var esQuery = {
            index: index_name,
            from: from,
            size: size,
    ...
    function bulk(record, type) {
        ...
        if (bulkQueue.length >= bulkSize) {
            logger.debug(`flushing bulk queue ${bulkQueue.size()}`);
            return _flush();
        }
    ...
    function isAvailable() {
        ...
        return new Promise(function(resolve, reject) {
            search()
                .then(function(results) {
                    logger.debug(`index ${index_name} is now available`);
                    resolve(results)
                })
                .catch(function(err) {
