...
module.exports = function(context, index_name, record_type, id_field, bulk_size) {
    ...
    function search(query, from, size, sort) {
        return client.search({
            ...
            var data = _.map(results.hits.hits, function(hit) {
                return hit._source
            });

            var currentNum = data.length + from;
            //set flag if not all data was gathered
            if (currentNum < results.hits.total) {
                data._remaining_data = {count: currentNum, total: results.hits.total};
            }
            return data;
        })
    }
