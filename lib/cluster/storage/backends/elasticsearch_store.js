...
module.exports = function module(context, indexName, recordType, idField, _bulkSize, fullResponse) {
    ...
    function migrate(indName) {
        ...
        return _createIndex(migrantIndexName)
            .then(() => client.reindex(reindexQuery))
            .then((results) => {
                if (results.failures.length > 0) {
                    logger.error(`There were ${results.failures.length} reindex errors from old index ${indName} to ${migrantIndexName}, errors: ${JSON.stringify(results.failures.length)}`);
                }
                return true;
                // for now we will manually delete the index until this is fleshed out
                // return client.indices.delete({ index: indName });
            })
