                ...
                .catch(() => {
                    const isReady = setInterval(() => {
                        if (shuttingDown) {
                            ...
                            const error = new Error(`Shutdown notice recieved when checking if index ${indexName} is available`);
                            logger.warn(error);
                            reject(error);
                            return;
                        }
            ...
            .catch((err) => {
                ...
                const checking = setInterval(() => {
                    if (shuttingDown) {
                        ...
                        const error = new Error(`Shutdown notice recieved when connecting to index ${indexName}`);
                        logger.warn(error);
                        reject(error);
                        return;
                    }
