...
module.exports = function module(context, indexName, recordType, idField, _bulkSize, fullResponse) {
    ...
    function destroy(timeoutMs = config.shutdown_timeout) {
        return new Promise((resolve) => {
            logger.info(`attempting to shutdown, will destroy in ${timeoutMs}`);
            const _destroy = () => {
                bulkQueue.length = 0;
                destroyFns.forEach((fn) => { fn(); });
                resolve();
            };
            const timeout = setTimeout(_destroy, timeoutMs);
            shutdown()
                .then(() => {
