...
module.exports = function(context, index_name, record_type, id_field, bulk_size, full_response) {
    ...
    var timeSeriesIndex = `${index_name.slice(0, index_name.length - 1)}-${new Date().toISOString().slice(0, 10).replace(/-/g, '.')}`;
    var newIndex = isMultiIndex ? timeSeriesIndex : index_name;
