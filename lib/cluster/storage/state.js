...
module.exports = function module(context) {
    ...
    function executionStartingSlice(exId, slicerId) {
        ...
        return backend.search(startQuery, 0, 1, 'slicer_order:desc')
            ...
            .catch((err) => {
                ...
                const error = new Error(`StateStorage: An error occurred getting the newest record, error: ${errMsg}`);
    ...
    function recoverSlices(exId, slicerId, cleanupType) {
        ...
        return backend.refresh(indexName)
            ...
            .catch((err) => {
                const errMsg = parseError(err);
                const error = new Error(`StateStorage: An error has occurred accessing the state log for retry, error: ${errMsg}`);
                logger.error(error.message);
                return Promise.reject(error);
            });
    }
