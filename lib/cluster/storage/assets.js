...
module.exports = function(context) {
            ...
            if (metaData.length === 1 || metaData[1] === 'latest') {
                ...
                return search(`name:${metaData[0]} AND version:${metaData[1]}`, null, 10000, `_created:desc`, ['version'])
                    .then(function(assetRecords) {
                        ...
                        if (records.length === 0) {
                            return Promise.reject(`error while attempting to load asset: ${metaData.join(':')}, either 
                            the asset does not exist or the version is incorrect`)
                        }
    ...
    function remove(asset_id) {
        return backend.remove(asset_id);
    }
