            ...
            .then(() => {
                ...
                return backend.get(id, null, ['name'])
                    ...
                    .catch((err) => {
                        ...
                        return Promise.reject(new Error(`error checking asset index, could not get asset with id: ${id}, error: ${parseError(err)}`));
                    });
            })
            .catch((fileError) => {
    ...
    function _getAssetId(assetIdentifier) {
        ...
        if (metaData.length === 1 || metaData[1] === 'latest') {
            return search(`name:${metaData[0]}`, null, 1, '_created:desc')
                .then((assetRecord) => {
                    ...
                    if (!record) {
                        return Promise.reject(new Error(`asset: ${metaData.join(' ')} was not found`));
                    }
                    return record._id;
                });
        }
        ...
        return search(`name:${metaData[0]} AND version:${metaData[1]}`, null, 10000, '_created:desc', ['version'])
            .then((assetRecords) => {
                ...
                if (records.length === 0) {
                    return Promise.reject(new Error(`No asset with the provided name and version could be located, asset: ${metaData.join(':')}`));
                }
    ...
    function _metaIsUnqiue(meta) {
        return search(`name:${meta.name} AND version:${meta.version}`, null, 10000)
            .then((results) => {
                ...
                return Promise.reject(new Error(`asset name:${meta.name} and version:${meta.version} already exists, please increment the version and send again`));
            });
    }
