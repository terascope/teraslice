...
module.exports = function _clusterMaster(context) {
    ...
    messaging.register({
        ...
        callback: () => {
            ...
            Promise.all(_.map(context.services, service => service.shutdown()))
                ...
                .then(() => process.exit(0))
                .catch((err) => {
                    const error = new VError(err, 'error while cluster_master is shutting down');
            ...
            if (assetsServiceIsReady) {
                ...
                const assetReady = setInterval(() => {
                    ...
                    if (assetsServiceError) {
                        clearInterval(assetReady);
                        reject(new Error('An error occurred while instantiating the assets service'));
                    }
                }, 100);
            }
        }));
    ...
    require('./services/execution.js')(context)
        ...
        .catch((err) => {
            const error = new VError(err, 'error during service initialization');
            logError(logger, error);
            return messaging.send({ to: 'node_master', message: 'cluster:error:terminal' });
        });
};
