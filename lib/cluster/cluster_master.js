...
module.exports = function(context) {
    var logger = context.foundation.makeLogger({module: 'cluster_master'});
    ...
    var assetsServiceIsReady = false;
    var assetsServiceError = false;
    logger.info(`listening on port ${clusterConfig.port}`);

    var messaging = messageModule(context, logger);
    context.messaging = messaging;

    messaging.register('assets:service:available', function(assetService) {
        if (assetService.error) {
