...
module.exports = function(context) {
    ...
    if (context.sysconfig.teraslice.master) {
        context.foundation.startWorkers(1, {module: 'cluster_master', node_id: context.sysconfig._nodeName});
    }

    var config = context.sysconfig.teraslice;
    var host = makeHostName(config.master_hostname, config.port);
    ...
    socket.on('cluster_service:create_slicer', function(slicerConfig) {
        var slicerContext = {
            assignment: 'slicer',
        ...
        socket.emit('node_master:message_processed', slicerConfig);
        ...
        if (!slicerFound) {
            var stop = setInterval(function() {
                ...
                if (workersForJob.length === 0) {
                    clearInterval(stop);
                    socket.emit('node_master:message_processed', data);
                }
    ...
    socket.on('cluster_service:remove_workers', function(data) {
    ...
    socket.on('connect_error', function(err) {
        logger.warn(`Attempting to connect to cluster_master: ${host}`)
    });
    ...
    function messageHandler(processMsg) {
        ...
        if (processMsg.message === 'slicer:slicer_initialized') {
            socket.emit('slicer:slicer_initialized', processMsg);
            return;
        }

        if (processMsg.message === 'slicer:job_finished') {
            socket.emit('slicer:job_finished', processMsg);
            return;
        }

        if (processMsg.message === 'job:terminal_error') {
            socket.emit('job:terminal_error', processMsg);
            return;
        }

        if (processMsg.message === 'message_processed') {
            socket.emit('node_master:message_processed', processMsg);
            return;
        }

        if (processMsg.message === 'slicer:failed_recovery') {
            socket.emit('slicer:failed_recovery', processMsg);
            return;
        }

        if (processMsg.message === 'slicer:processing_error') {
            socket.emit('slicer:processing_error', processMsg);
            return;
        }

        if (processMsg.message === 'slicer:job_update') {
            socket.emit('slicer:job_update', processMsg);
            return;
        }
