...
module.exports = function module(context) {
    ...
    // TODO: sendNodeState was put in meesageModule
    ...
    messaging.register('network:connect', () => {
        ...
        console.log('sending node state');
    ...
    // TODO: can this be extracted out
    messaging.register('assets:loaded', (assetsMsg) => {
        messageWorkers(assetsMsg, assetsMsg, (worker) => {
    ...
    // TODO: see if we can abstract this
    messaging.register('assets:preloaded', (assetsMsg) => {
        messaging.broadcast('node:message:processed', assetsMsg);
    });
    ...
    messaging.register('cluster:slicer:analytics', (data) => {
        messageWorkers(data, {
    ...
    messaging.register('cluster:slicer:create', (createSlicerRequest) => {
        ...
        messaging.broadcast('node:state', getNodeState());
        messaging.broadcast('node:message:processed', createSlicerRequest);
    });

    messaging.register('cluster:workers:create', (createWorkerRequest) => {
        console.log('am i creating workers in node master?');
        ...
        messaging.broadcast('node:message:processed', createWorkerRequest);
    });
    ...
    messaging.register('cluster:job:stop', (data) => {
        let slicerFound = false;
        let stopTime = config.network_timeout;
        const intervalTime = 200;

        messageWorkers(data, { message: 'worker:shutdown' }, (worker) => {
        ...
        if (!slicerFound) {
            const stop = setInterval(() => {
                ...
                if (workersForJob.length === 0) {
                    ...
                    messaging.broadcast('node:message:processed', data);
                }
    ...
    messaging.register('cluster:job:pause', (data) => {
        messageWorkers(data, { message: 'cluster:job:pause' }, worker => worker.ex_id === data.ex_id && worker.assignment === 'slicer');
    });

    messaging.register('cluster:job:resume', (data) => {
        messageWorkers(data, { message: 'cluster:job:resume' }, worker => worker.ex_id === data.ex_id && worker.assignment === 'slicer');
    });
    // TODO : review the newWorkerQueue
    messaging.register('cluster:job:restart', (data) => {
        messageWorkers(data, { message: 'cluster:job:restart' },
            worker => worker.ex_id === data.ex_id && worker.assignment === 'slicer',
            (worker) => {
    ...
    messaging.register('cluster:workers:remove', (data) => {
    ...
    messaging.register('cluster:node:get_port', (msg) => {
        ...
        messaging.broadcast('node:message:processed', msg);
    });

    // TODO: check to see if we need to stop job
