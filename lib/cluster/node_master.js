...
module.exports = function(context) {
    ...
    messaging.register('cluster:job:stop', function(data) {
        ...
        messageWorkers(context, data, {message: 'cluster:job:stop'}, function(worker) {
    ...
    messaging.register('node:connection_error', function(err) {
    ...
    function messageHandler(processMsg) {
        ...
        if (processMsg.message === 'slicer:slicer_initialized') {
            messaging.send('slicer:slicer_initialized', processMsg);
            return;
        }

        if (processMsg.message === 'slicer:job:finished') {
            console.log('am i getting called here when all is said and done in node master for a job');
            messaging.send('slicer:job:finished', processMsg);
            return;
        }

        if (processMsg.message === 'job:terminal_error') {
            messaging.send('job:terminal_error', processMsg);
            return;
        }

        if (processMsg.message === 'message_processed') {
            messaging.send('node:message:processed', processMsg);
            return;
        }

        if (processMsg.message === 'slicer:failed_recovery') {
            messaging.send('slicer:failed_recovery', processMsg);
            return;
        }

        if (processMsg.message === 'slicer:processing_error') {
            messaging.send('slicer:processing_error', processMsg);
            return;
        }

        if (processMsg.message === 'slicer:job_update') {
            messaging.send('slicer:job_update', processMsg);
            return;
        }
        if (processMsg.message === 'cluster:error:terminal') {
    ...
    messaging.register('child:online', function(worker) {
