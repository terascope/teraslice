...
module.exports = function module(context) {
    ...
    function messageWorkers(clusterMsg, processMsg, filterFn) {
        ...
        _.each(allWorkersForJob, (worker) => {
            if (context.cluster.workers[worker.worker_id]) {
                logger.trace(`sending message to worker ${worker.worker_id}, ex_id: ${worker.ex_id}, msg:`, processMsg);
                context.cluster.workers[worker.worker_id].send(processMsg);
            }
        });
    }
    ...
    function killWorkers(filterFn) {
        ...
        _.each(allWorkersForJob, (worker) => {
            if (context.cluster.workers[worker.worker_id]) {
                ...
                context.cluster.workers[worker.worker_id].process.kill('SIGKILL');
            }
        });
    }
