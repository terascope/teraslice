...
module.exports = function(context) {
    ...
    var messaging = messageModule(context, logger, null, sendNodeState);
    var host = messaging.getHostUrl();

    logger.info(`node ${context.sysconfig._nodeName} is attempting to connect to cluster_service: ${host}`);
    ...
    messaging.register('assets:preloaded', function(assetsMsg) {
        messageWorkers(context, assetsMsg, assetsMsg, function(worker) {
            return worker.assignment === 'jobs_service';
        });
    });

    messaging.register('service', function(serviceMsg) {
        messageWorkers(context, serviceMsg, serviceMsg, function(worker) {
            return worker.assignment === serviceMsg.service;
        });
    });
    ...
    messaging.register('cluster:slicer:create', function(createSlicerMsg) {
        ...
        //need to update cluster_service immediately so it can balance workers correctly and prevent wrong allocations
        messaging.send('node:state', getNodeState());
    ...
    messaging.register('network:error', function(err) {
        logger.warn(`Attempting to connect to cluster_service: ${host}`, err)
    });

    messaging.register('cluster:error:terminal', function() {
        logger.error('terminal error in cluster_service, flushing logs and shutting down');
    ...
    if (context.sysconfig.teraslice.master) {
        ...
        logger.debug(`node ${context.sysconfig._nodeName} is creating the api_service`);
        context.foundation.startWorkers(1, {
            assignment: 'api_service',
            assets_port: assets_port,
            node_id: context.sysconfig._nodeName
        });

        logger.debug(`node ${context.sysconfig._nodeName} is creating the cluster_service`);
        context.foundation.startWorkers(1, {
            assignment: 'cluster_service',
            assets_port: assets_port,
            node_id: context.sysconfig._nodeName
        });

        logger.debug(`node ${context.sysconfig._nodeName} is creating the jobs_service`);
        context.foundation.startWorkers(1, {
            assignment: 'jobs_service',
            node_id: context.sysconfig._nodeName
        });
