...
module.exports = function(context) {
    ...
    socket.on('cluster_service:stop_job', function(data) {
        ...
        messageWorkers(context, data, {message: 'cluster_service:stop_job'}, function(worker) {
            ...
            return worker.jx_id === data.jx_id
        });
    ...
    socket.on('cluster_service:pause_slicer', function(data) {
        messageWorkers(context, data, {message: 'cluster_service:pause_slicer'}, function(worker) {
            return worker.jx_id === data.jx_id && worker.assignment === 'slicer'
        });
    });
    ...
    socket.on('cluster_service:remove_workers', function(data) {
        ...
        var workerCount = _.filter(children.active, function(worker) {
            return worker.jx_id === data.jx_id && worker.assignment === 'worker';
        }).length;

        messageWorkers(context, data, {message: 'shutdown'},
            function(worker) {
                if (worker.jx_id === data.jx_id && worker.assignment === 'worker' && counter) {
                    counter -= 1;
                    return worker;
                }
            });
