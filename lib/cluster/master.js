...
function getNodeState(context) {
    var state = {id: context.sysconfig._nodeName, total: context.sysconfig.terafoundation.workers, hostname: context.sysconfig.teraslice.hostname};
    var clusterWorkers = context.cluster.workers;
    var active = [];

    for (var childID in clusterWorkers) {
        var child = {
            id: clusterWorkers[childID].id,
            assignment: clusterWorkers[childID].assignment,
            pid: clusterWorkers[childID].process.pid
        };

        if(clusterWorkers[childID].jobID){
            child.jobID = clusterWorkers[childID].jobID
        }
...
module.exports = function(context) {
    ...
    socket.on('create slicer', function(msg) {
        context.foundation.startWorkers(1, 'slicer', {job: msg.job});
    ...
    socket.on('pause slicer', function(data){
        data.forEach(function(worker){
            sendProcessMessage(cluster, worker.id, {message: 'pause'})
        })
    });

    socket.on('resume slicer', function(data){
        data.forEach(function(worker){
            sendProcessMessage(cluster, worker.id, {message: 'resume'})
        })
    });

    socket.on('restart slicer', function(data){

        //TODO verify slicer shutdown and new slicer timing
        data.forEach(function(worker){
            sendProcessMessage(cluster, worker.id, {message: 'exit for retry'})
        });

        data.forEach(function(worker){
            context.foundation.startWorkers(1, 'slicer', {job: cluster.workers[worker.id].job, jobRetry: true});
        });
    });
