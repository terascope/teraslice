...
module.exports = function(context) {
    ...
    function initializeNetwork() {
        ...
        //this signals that worker was successfully created in regards to job allocation
        process.send({
            message: 'message_processed',
            _msgID: process.env._msgID
        });
    ...
    function isReady(res, slice, msg, specData) {
        ...
        if (res && res.errors) {
            ...
            logState(job.jobConfig, slice, 'completed').then(function() {
    ...
    function runSlice(slice) {
        ...
        function sliceFailed(err) {
            ...
            logState(job.jobConfig, slice, 'error', err).then(function() {
                socket.emit('worker:slice_complete', sentMessage);
            });
        }
