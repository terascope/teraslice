...
module.exports = function(context) {
    ...
    messaging.register('assets:loaded', function(ipcMessage) {
        event.emit('worker:assets_loaded', ipcMessage)
    });
    ...
    function isReady(res, slice, msg, specData, sliceLogger) {
        ...
        return state_store.log(ex_id, slice, 'completed')
            .then(function(results) {
                event.emit('slice:success', slice);
    ...
    function runSlice(slice, sliceLogger) {
        ...
        function sliceFailed(err) {
            ...
            event.emit('slice:failure', sentMessage);
    ...
    function instantiateJob() {
        return new Promise(function(resolve, reject) {
            var job_runner = require('./runners/job')(context, event);
            var job;

            event.on('worker:assets_loaded', function(ipcMessage) {
                if (ipcMessage.error) {
