...
module.exports = function module(context) {
    ...
    messaging.register({
        event: 'assets:loaded',
        callback: ipcMessage => events.emit('worker:assets_loaded', ipcMessage)
    });
    ...
    messaging.register({
        ...
        callback: () => {
            if (sentMessage) {
                ...
                messaging.send({ to: 'slicer', message: 'worker:slice:complete', worker_id: ID, payload: sentMessage });
            } else {
                logger.debug(`a worker has made the initial connection to slicer ex_id: ${exId}`);
                messaging.send({ to: 'slicer', message: 'worker:ready', worker_id: ID, payload: { worker_id: ID } });
            }
        }
    });
