...
module.exports = function module(context) {
    ...
    function isReady(res, slice, msg, specData, sliceLogger) {
        ...
        return stateStore.updateState(slice, 'completed', slice.index)
            .then(() => {
                ...
                if (specData) {
                    if (job.reporter) {
                        job.reporter(context, job.config, specData);
    ...
    function recycleFn(jobSpec) {
        let sliceCount = 0;
        const recycleWorker = jobSpec.config.recycle_worker;
        return function () {
            if (recycleWorker) {
                sliceCount += 1;
                // recycle worker between 75% and 100% of job.config.recycle_worker
