...
module.exports = function(context) {
    ...
    function runSlice(slice) {
        ...
        function sliceFailed(err) {
            ...
            logState(job.jobConfig, slice, 'error', err)
                .then(function() {
                    logger.error(`worker: ${ID} has failed to process`, sentMessage, `and has slice state is marked as error`);
                    socket.emit('worker:slice_complete', sentMessage);
                })
                .catch(function(err) {
                    logger.error('An error has occured: ' + JSON.stringify(err) + ' on marking slice as failed, message: ', slice, err);
                });
        }
        ...
        work.then(function(res) {
            ...
            .catch(function(err) {
                logger.error('An error has occured: ' + JSON.stringify(err) + ' on message: ', slice, err.stack);
                if (max_retries) {
