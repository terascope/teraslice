...
module.exports = function(context) {
    ...
    var network = messaging({
        type: 'network',
        isClient: true,
        host: job.jobConfig.slicer_hostname,
        port: job.jobConfig.slicer_port,
        networkConfig: {reconnect: true},
        context:context
    });

    var ipc = messaging({type: 'process', isClient: true, context:context});
    ...
    function initializeNetwork() {
        ...
        //set up listener
        network.register('slicer:new_slice', function(msg) {
            isDone = false;
        ...
        network.register('slicer:slice_recorded', function() {
            //the sent message has been processed, so set it to false;
            sentMessage = false;
        });

        network.register('error', function(err) {
            logger.error('Error in worker socket: ', err)
        });

        network.register('disconnect', function(e) {
            logger.error(`getting a socket disconnect event`, e);
        });
        ...
        network.register('connect', function() {
            if (sentMessage) {
                ...
                network.send('worker:slice_complete', sentMessage);
            }
            else {
                network.send('worker:ready', {id: ID});
            }
        });
