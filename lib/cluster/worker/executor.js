...
module.exports = function module(context, messaging, stateStore, analyticsStore) {
    const events = context.apis.foundation.getSystemEvents();
    const { cluster, sysconfig: { teraslice: config } } = context;
    let { ex_id: exId, job_id: jobId } = process.env;
    ...
    function _retrySliceModule(slice, operations, sliceLogger, specData) {
        const { max_retries: maxRetries } = executionContext.config;
        let retryCount = 0;
    ...
    function shutdown() {
        let { shutdown_timeout: counter } = config;
        isShuttingDown = true;
    ...
    function _recycleFn() {
        const { recycle_worker: recycleWorker } = executionContext.config;
    ...
    function testContext(_executionContext, _exId, _jobId) {
        isTesting = true;
        if (_exId) exId = _exId;
