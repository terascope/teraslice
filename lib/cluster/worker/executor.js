...
module.exports = function module(context, messaging, stateStore, analyticsStore) {
    ...
    function _recycleFn(recycleWorker) {
        const recycleWorkerRandomFactor = _.random(75, 100);
        let sliceCount = 0;

        return function __recycleFn() {
    ...
    function testContext(_executionContext, _exId, _jobId, _sentMessage) {
        // reset state
        sentMessage = false;
        isShuttingDown = false;
        isDone = true;
        isTesting = true;
        if (_sentMessage) sentMessage = _sentMessage;
        if (_exId) exId = _exId;
        ...
        recycle = _recycleFn(executionContext.config.recycle_worker);
        return {
