...
module.exports = function module(context, messaging, stateStore, analyticsStore, options = {}) {
    ...
    const executionRunner = require('../runners/execution')(context, options);
    ...
    messaging.register({
        ...
        callback: () => {
            const msgHasBeenSent = _.isEqual(_cachedSentMessage, sentMessage);
            if (sentMessage && !msgHasBeenSent) {
                logger.warn('reconnecting to slicer, previous slice: ', sentMessage);
                sentMessage.retry = true;
                _cachedSentMessage = _.cloneDeep(sentMessage);
                ...
                messaging.send({
                    ...
                    logger.error('waiting for slice completed response', err);
                    _cachedSentMessage = null;
                });
            } else {
    ...
    function initialize() {
        return executionRunner.initialize(events, logger)
            .then((_executionContext) => {
                ...
                messaging.listen({ query: { worker_id: ID } });
