    ...
    Promise.resolve(require('./storage/assets')(context))
        ...
        .then((assetArray) => {
            ...
            if (process.env.preload) {
                return messaging.respond(respondingData, { message: 'assets:preloaded', meta: assetArray });
            }
        ...
        .catch((err) => {
            ...
            if (process.env.preload) {
                return messaging.respond(respondingData, { message: 'assets:preloaded', error: errMsg });
            }
            return messaging.send({
