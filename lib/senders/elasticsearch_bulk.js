...
function newSender(context, opConfig, jobConfig) {
    ...
    function filterResponse(data, results) {
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            if (item.error) {
                ...
                if (item.error.type === 'es_rejected_execution_exception') {
                    logger.warn('The elasticsearch cluster queues are overloaded, resubmitting failed queries from bulk')
                    if (i === 0) {
                ...
                function send(data) {
                    client.bulk({body: data})
                        .then(function(results) {
                            if (results.errors) {
                                var response = filterResponse(data, results);
