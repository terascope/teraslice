...
function getIndexDate(opConfig, client) {
    var query = {
        ...
        body: {
            aggs: {
                min_date: {
                    min: {
                        field: opConfig.date_field_name
                    }
                },
                max_date: {
                    max: {
    ...
    return client.search(query).then(function(data) {
        var start = data.aggregations.min_date.value_as_string;
        var end = data.aggregations.max_date.value_as_string;

        return {start: start, end: end};
    }).catch(function(e) {
        throw new Error('Could not retrieve min and max date for index', e)
    })
}

function getDates(opConfig, client) {
    var configStart = opConfig.start;
    var configEnd = opConfig.end;

    return Promise.resolve(getIndexDate(opConfig, client))
        .then(function(data) {

            if (configStart === null) {
                configStart = data.start
            }
            if (configEnd === null) {
                //end datw is non-inclusive, adding 1s so range will cover it
                var time = moment(data.end).add(1, 's');
                configEnd = time.format(dateFormat)
            }

            return {start: parseDate(configStart), limit: parseDate(configEnd)}
        });
}
