    ...
    return function() {
        if (!isProcessing) {
            ...
            if (retryQueue && retryQueue.size()) {
                ...
                console.log('in the else statemnet', slicer);
                Promise.resolve(slicer(worker))
                    .then(function(data) {
                        //not null or undefined
                        console.log('the slicer is calling');
                        if (data != null) {
                            writeToStateFile(job.stateName, data, 'start');
                            console.log('getting data past the statefilewrite', data);
                            sendMessage(worker.id, {message: 'data', data: data});
                            console.log('should have sent', worker.id);
                        }
                        else {
                            if (clusterShutdown) {
