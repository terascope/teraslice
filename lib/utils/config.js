...
function writeToStateFile(stateName, msg) {
    fs.appendFileSync(stateName, JSON.stringify(msg) + '\n')
}

function stateLog(context, job) {
    var name = "__" + job.name.replace(' ', '') + 'State';
    var path = context.sysconfig.terafoundation.log_path + "/" + name;
...
function getStartFromLog(stateName) {
    var tail = cp.execSync('tail -n 30 ' + stateName);
    var data = String(tail).trim().split('\n');

    var results = data.reduce(function(prev, accum) {
        try {
            var slice = JSON.parse(accum);
            return compareDates(prev, slice)
        }
        catch (e) {
            return prev
        }
...
function initializeJob(context) {
    ...
    var stateName = stateLog(context, job);

    //generate workers
    if (context.cluster.isMaster && !context.sysconfig.teraslice.slicer) {
        initializeWorkers(context, validJob);
    }

    if (argv.retry) {
        var newStart = getStartFromLog(stateName);
    }
    ...
    job.operations.forEach(function(jobConfig, i) {
        ...
        if (i === 0) {
            ...
            if (newStart) {
                readerConfig.start = newStart.end
            }
...
function persistent(slicer, start, workerQueue, jobConfig, sendMessage, analyticsData, distConfig, terminateWorker) {
    var nextSlice = slicer;
    var start = start;
    var logger = _jobConfig.logger;
    var isProcessing = false;
...
function once(slicer, start, workerQueue, jobConfig, sendMessage, analyticsData, distConfig, terminateWorker) {
    var nextSlice = slicer;
    var start = start;
    var logger = _jobConfig.logger;
    var isProcessing = false;
            ...
            Promise.resolve(nextSlice(worker))
                .then(function(data) {
                    ...
                    if (data != null) {
                        ...
                        //ending sequence for distributed mode
                        if (distConfig) {
                            //need to eventually kill worker
                            // process.send({message: 'terminate_child_process', id: _context.cluster.worker.id});
                            terminateWorker(_, worker.id, {message: 'data', data: data})
                        }
                        //ending sequence for single mode
                        else {
                            processDone++;
