...
function initializeJob(context) {
    ...
    var job = process.env.job ? JSON.parse(process.env.job) : getJob();
    var reader;
    var sender;
    var readerConfig;
    var senderConfig;
...
function validateNumOfWorkers(context) {
    var job = validateOperation(context, jobSchema, getJob(), false);
    ...
    if (job.workers > sysWorkers) {
        logger.warn(' The number of workers specified on the job ( ' + job.workers + ' ) is higher than what has ' +
            'been allocated in the config file at terafoundation.workers ( ' + sysWorkers + ' ). The number of workers ' +
