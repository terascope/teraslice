...
function getStartFromLog(stateName) {
    ...
    console.log('i got a value', startFrom, typeof startFrom);
...
function once(slicer, start, workerQueue, job, analyticsData) {
    var logger = job.jobConfig.logger;
    var isProcessing = false;
    var errorQueue = [];
    ...
    return function() {
        ...
        if (!isProcessing) {
            ...
            if (retryQueue && retryQueue.size()) {
                ...
                writeToStateFile(job.stateName, retryData, 'start');
                sendMessage(_context.cluster, worker.id, {message: 'data', data: retryData})
            }
            else {
