...
function persistent(slicer, workerQueue, job, sendMessage, writeToStateFile, io, event) {
...
function once(slicer, workerQueue, job, sendMessage, writeToStateFile, io, event) {
    var isProcessing = false;
                ...
                Promise.resolve(slicer(worker))
                    .then(function(data) {
                        ...
                        if (data != null) {
                            writeToStateFile(job.stateName, data, 'start');
                            sendMessage(worker.id, {message: 'data', data: data});
                            console.log('its sent to worker', worker.id);
                        }
                        else {
                            processDone++;
                            if (processDone === io.sockets.sockets.length) {
