...
var commonSchema = require('../config/schemas/job').commonSchema;
...
function getJobWithID(context) {
    ...
    if (retry) {
        ...
        //generate an id
        jobWithID = addID(job);
        //TODO add port here to job
    }
    logger.info('Job: ', job.name, ' has been initiated, id: ', job.job_id);

    //return {job_id: jobWithID.job_id, jobStr: JSON.stringify(jobWithID)};
...
function makeLogName(context, job, suffix) {
    ...
    var fullName = name + '_' + job.job_id + '-' + job.name + '__' + suffix;
...
function initializeJob(context) {
    ...
    var state_store = require('../cluster/storage/state')(context);
    ...
    return state_store.buildRetryQueue(job.job_id)
...
function persistent(context, slicer, workerQueue, job, sendMessage, state_store, io, event) {
...
function once(context, slicer, workerQueue, job, sendMessage, state_store, io, event) {
    var isProcessing = false;
