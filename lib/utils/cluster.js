...
function distributeJob(context, clusterState, io, res, job, jobRequest) {
    ...
    if (canDistributeJob) {
        ...
        jobRequest[job.job_id] = {requestedWorkers: numOfWorkers, job: jobStr};
        var dispatch = allocateWorkers(clusterState, jobRequest, sortedNodes, job.job_id);

        sendMessage(io, slicerNodeID, 'create slicer', {job: jobStr, job_id: job_id});
        jobRequest[job.job_id].requestedWorkers -= 1;

        _.each(dispatch, function(node) {
            jobRequest[job.job_id].requestedWorkers -= node.workers;
            sendMessage(io, node.key, 'create workers', {workers: node.workers, job: jobStr, job_id: job_id})
        });

        res.status(202).json({id: job.job_id, logs_index: jobName})
...
function findNodesWithJob(clusterState, id, bool) {
    var nodes = [];
...
function jobAction(req, res, io, clusterState, jobRequest) {
    ...
    if (message === 'stop job') {
        //remove any outstanding requests for workers for a stop job
        delete jobRequest[job_id];
        nodes = findNodesWithJob(clusterState, job_id, true);
    }
    else {
        nodes = findNodesWithJob(clusterState, job_id);
    }

    if (nodes.length === 0) {
        //if there are no active jobs, need
        if (action === 'restart') {
            //TODO get job from job DB
            //distributeJob with retry ability
        }
        ...
        nodes.forEach(function(node) {
            io.sockets.in(node.node_id).emit(message, node)
        });
