...
function distributeJob(clusterState, io, jobStr, res, jobRequest) {
    ...
    if (canDistributeJob(clusterState)) {
        ...
        res.send("job had been accepted, id: " + job.id)

    }
    else {
        //TODO need to add enqueueing logic
        res.stats(200).send(' There are not enough workers available, job is enqueued');
        setTimeout(distributeJob(clusterState, io, job, res, jobRequest), 5000)
    }
