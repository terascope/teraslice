...
function findNodesWithJob(clusterState, id) {
    ...
    _.forOwn(clusterState, function(node) {
        var hasJob = node.active.filter(function(worker) {
            return worker.jobID === id && worker.assignment === 'slicer';
        });
...
function jobAction(req, res, io, clusterState) {
    ...
    var nodes = findNodesWithJob(clusterState, jobID);

    if (nodes.length === 0) {
        res.status(404).send('No active job was found with the provided id: ' + jobID +' \n the job may ' +
...
//verify job exists
//verify action
    //send message to node_master to pause slicer and workers
