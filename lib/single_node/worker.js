...
module.exports = function(context) {
    ...
    var host = makeHostName(job.jobConfig.slicer_hostname, job.jobConfig.slicer_port);
    var client;
    var opConfig;
...
//TODO catch needs to be changed to correct sender
    ...
    function isReady(res, msg, specData) {
        ...
        if (res && res.errors) {
            ...
            socket.emit('slice complete', {id: ID, slice: msg, analytics: specData});
            isDone = true;
        }
    }
    ...
    function runSlice(msg) {
        ...
        work.then(function(res) {
            ...
            .catch(function(err) {
                logger.error('the crazy error', err)
                ...
                if (max_retries) {
                    ...
                    if (errorLog[msg]) {
                        ...
                        if (errorLog[msg] >= max_retries) {
                            ...
                            socket.emit('slice complete', {id: ID, slice: msg, error: true});
                            isDone = true;
                        }
                        else {
                            runSlice(msg);
    ...
    process.on('message', function(msg) {
        if (msg.message === 'shutdown'){
            console.log('getting a shutdown', ID);
        finalShutdown();
        }
    });
