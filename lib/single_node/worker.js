...
module.exports = function(context) {
    ...
    var createNameSpace = require('../utils/cluster').createNameSpace;
    ...
    var nameSpace = createNameSpace(job.jobConfig);

    var host = makeHostName(config.host, config.port);

    console.log('host from worker', host);
...
//TODO catch needs to be changed to correct sender
    ...
    process.on('message', function(msg) {
        if (msg.message === 'shutdown') {
            console.log('getting a shutdown msg', context.cluster.worker.id);
            finalShutdown();
        }
    });
