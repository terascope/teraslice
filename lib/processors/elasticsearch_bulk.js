                        ...
                        if (results.errors) {
                            ...
                            if (response.error) {
                                ...
                                if (response.data.length === 0) {
                                    ...
                                    //TODO need future moderator here
                                    warning();
                                    setTimeout(function() {
    ...
    function multiSend(data) {
        ...
        for (var i = 0; i < data.length;) {
            ...
            if (isMetaData && isMetaData.meta._id) {
                ...
                if (bulk_contexts.hasOwnProperty(selector)) {
                    bulk_contexts[selector].data.push(data[i]);
                    i += 1;

                    if (isMetaData.type !== 'delete') {
                        bulk_contexts[selector].data.push(data[i + 1]);
                        i += 1;
                    }
                }
                else if (bulk_contexts.hasOwnProperty('*')) {
                    bulk_contexts['*'].data.push(data[i]);
                    i += 1;
