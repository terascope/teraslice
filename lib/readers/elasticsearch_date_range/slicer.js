...
function newSlicer(context, opConfig, job, retryData, client) {
    ...
    function getIndexDate(opConfig, client, date, order) {
        ...
        return client.search(query).then(function(data) {
            ...
            if (data.hits.hits.length === 0) {
                throw new Error('No data found, please check the query parameter, start/end dates and/or date-field_name in elasticsearch_reader config');
            }

            if (data.hits.hits[0]._source[opConfig.date_field_name] === undefined) {
                throw new Error(`date_field_name: "${opConfig.date_field_name}" for index: ${opConfig.index} does not exist`);
            }
    ...
    function updateJob(opConfig, dates) {
        ...
        if (update.hasOwnProperty('start') || update.hasOwnProperty('end')) {
            ...
            process.send({message: 'slicer:job_update', update: operations, ex_id: process.env.ex_id})
        }
    }
            ...
            if (count > dateParams.size) {
                ...
                if (isExpandedSlice) {
                    ...
                    if (diff <= 1) {
                        ...
                        logger.info(`slicer: ${slicer_id} is recursing ${JSON.stringify(dateParams)}`);
