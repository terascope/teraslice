...
function newSlicer(context, opConfig, job, retryData, logger, client) {
            ...
            .then(function(startDate) {
                return Promise.resolve(getIndexDate(opConfig, opConfig.end, 'end'))
                    .then(function(endDate) {
                        ...
                        logger.info(`slicer: ${job.config.ex_id} start and end times are ${JSON.stringify(finalDates)}`);
                        return finalDates;
                    });
            });
    }
    ...
    function makeKeyList(opConfig, data) {
        ...
        const idSlicer = require('../id_slicer')(client, job, idConfig, logger, null, data);
    ...
    function divideRange(start, end, job) {
        var numOfSlicers = job.config.slicers;
        var results = [];
