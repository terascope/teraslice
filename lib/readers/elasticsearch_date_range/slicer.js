...
var elasticsearchVersion = require('../../cluster/storage/backends/elasticsearch').elasticsearchVersion;
var autoDates = ('../../cluster/storage/backends/elasticsearch').autoDates;
var queryCount = ('../../cluster/storage/backends/elasticsearch').queryCount;
...
function newSlicer(context, opConfig, job, retryData, logger, client) {
    ...
    function checkElasticsearch(client, opConfig, logger) {
        if (client) {
            return elasticsearchVersion(client, opConfig, logger)
        }
        else {
    ...
    function getIndexDate(opConfig, client, date, order) {
        ...
        return autoDates(client, opConfig, query, givenDate, order, dateFormat, logger);
    }
    ...
    function getCount(opConfig, dates, key) {
        ...
        //duplication
        return queryCount(client, query, logger);
    }
