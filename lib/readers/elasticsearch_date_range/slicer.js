...
function newSlicer(context, opConfig, executionContext, retryData, logger, client) {
    ...
    var executionConfig = executionContext.config;
    var isPersistent = executionConfig.lifecycle === 'persistent';
    var slicers = [];
    var time_resolution = dateOptions(opConfig.time_resolution);
    var retryError = retryModule(logger, executionConfig.max_retries);
    ...
    function getIndexDate(opConfig, date, order) {
        ...
        return elasticsearch.search(query)
            .then(function(results) {
                ...
                if (order === 'start') {
                    ...
                    var time = moment(date).add(1, time_resolution);
    ...
    function getDates(context, opConfig) {
        return Promise.resolve(getIndexDate(opConfig, opConfig.start, 'start'))
            .then(function(startDate) {
                return Promise.resolve(getIndexDate(opConfig, opConfig.end, 'end'))
        ...
        return getCount(config, dateParams).then(function(count) {
            if (count > dateParams.size) {
                ...
                if (isExpandedSlice) {
                    ...
                    var clonedParams = { interval: dateParams.interval };
    ...
    function nextChunk(opConfig, client, executionConfig, dates, slicer_id, retryData) {
    ...
    function divideRange(start, end) {
        var numOfSlicers = executionConfig.slicers;
        var results = [];
    ...
    function getTimes(opConfig, executionConfig) {
        ...
        var dateArray = divideRange(delayedStart, delayedEnd, {executionConfig: executionConfig});
    ...
    function awaitChunk(opConfig, client, executionConfig, slicerDates, slicer_id) {
        var shouldDivideByID = opConfig.subslice_by_key;
        var threshold = opConfig.subslice_key_threshold;
    ...
    if (isPersistent) {
        var dataIntervals = getTimes(opConfig, executionConfig);

        dataIntervals.forEach(function(dates, index) {
            slicers.push(awaitChunk(opConfig, client, executionConfig, dates, index));
        });
                    ...
                    .then(function(esDates) {
                        ...
                        return getInterval(opConfig, esDates)
                            .then(function(interval) {
                                ...
                                var dateRange = divideRange(esDates.start, esDates.limit);
                                //nextChunk pulls config off of opConfig, not executionConfig so executionConfig is not mutated
                                opConfig.interval = interval;
                                updateJob(opConfig, esDates);
                                dateRange.forEach(function(dates, index) {
                                    slicers.push(nextChunk(opConfig, client, executionConfig, dates, index, retryData[index]));
                                });
