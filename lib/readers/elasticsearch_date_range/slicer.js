...
function newSlicer(context, opConfig, job, retryData, logger, client) {
            ...
            if (count > dateParams.size) {
                ...
                if (isExpandedSlice) {
                    ...
                    if (diff <= 1) {
                        ...
                        logger.debug(`slicer: ${slicer_id} is recursing ${JSON.stringify(dateParams)}`);
    ...
    function makeKeyList(opConfig, data) {
        ...
        console.log('should start recuring', transformedKeyArray);
        function recurse(arr) {
            console.log('in the recurse fn');
            return Promise.all(arr.map(function(key) {
                console.log('what key', data, key);
                return getCount(opConfig, data, key)
                        .then(function(count) {
                            console.log('anthing after the count');
                ...
                logger.error('the erres lolz',err);
            });
        }
                ...
                return determineSlice(client, opConfig, dateParams, slicer_id, false)
                    .then(function(data) {
                        ...
                        var otherdata = {
                            ...
                            end: dateParams.limit,
                            count: 200000
                        };

                        if (shouldDivideByID) {
                            console.log('should be in here');
                            return Promise.resolve(makeKeyList(opConfig, otherdata))
                                .then(function(results) {
                    ...
                    .catch(function(err) {
                        //TODO need to flesh out what this will do, probably needs moderator
            ...
            if (dateParams.start.isSameOrAfter(limit)) {
                ...
                return determineSlice(client, opConfig, dateParams, slicer_id, false)
                    .then(function(data) {
                            ...
                            if (shouldDivideByID && data.count >= threshold && data.count >= opConfig.size * multiplier) {
    ...
    function getCount(opConfig, dates, key) {
        if(key){
            console.log('in the getcount', key);
        ...
        if(key){
            console.log('in the getcount', query, key);
        ...
        return new Promise(function(resolve, reject) {
            client.search(query)
                .then(function(results) {
                    console.log('got a reuls',key,  results.hits.total);
