...
function newSlicer(context, job, retryData, slicerAnalytics, logger, event) {
    ...
    return require('./elasticsearch_date_range/slicer.js')(context, opConfig, job, retryData, logger, client, event);
}

function newReader(context, opConfig, jobConfig, event) {
    var client = getClient(context, opConfig, 'elasticsearch');

    return require('./elasticsearch_date_range/reader.js')(context, opConfig, jobConfig, client, event);
}
