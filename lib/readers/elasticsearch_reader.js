...
function nextChunk(opConfig, client, jobConfig, dates, retryData) {
    ...
    if (retryData && retryData.lastSlice && retryData.lastSlice.end) {
        start = moment(retryData.lastSlice.end);
    }
...
function awaitChunk(opConfig, client, jobConfig, slicerDates) {
    ...
    var start = moment(slicerDates.start);
    var startPoint = moment(slicerDates.start);
    var end = moment(slicerDates.end);
    ...
    return function(msg) {
        if (start.isSameOrAfter(limit)) {
            ...
            if (dateArray.length > 0) {
                ...
                start = moment(newRange.startPoint);
                end = moment(newRange.limit);
            }
            return null;
        }
        else {
