...
var dateTimeParser = require('datemath-parser');
...
function newSlicer(context, job, retryData) {
    ...
    return require('./elasticsearch_date_range/slicer.js')(context, opConfig, job, retryData, client);
}
...
function op_validation(op) {
    if (op.subslice_by_key) {
        if (!op.type) {
            throw new Error('If subslicing_by_key is set to true, the elasticsearch type parameter of the documents must also be set')
        }
    }
}
