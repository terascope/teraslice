...
function newReader(context, opConfig) {
    const schema = parsedSchema(opConfig);
    ...
    return function (msg) {
        if (opConfig.stress_test) {
            ...
            const data = mocker()
                ...
                .build(newData => newData.schema[0]);
        ...
        return mocker()
            ...
            .build(data => data.schema);
    };
}

function onceGenerator(context, opConfig, jobConfig) {
    let numOfRecords = opConfig.size;
    const lastOp = jobConfig.operations[jobConfig.operations.length - 1];
...
function newSlicer(context, job) {
    const jobConfig = job.jobConfig;
    const opConfig = getOpConfig(jobConfig, 'elasticsearch_data_generator');
    const slicers = [];
