...
describe('Parser', () => {
    ...
    describe('when given a invalid function query "location:geoPolygon(points:[["123.43,223.43", "102.3,123.4"], "99.3,154.4" ])", it can still parse array of arrays but break validation', () => {
        it('should throw an error', () => {
            const errMsg = 'Failure to parse xlucene query "location:geoPolygon(points:[["123.43,223.43", "102.3,123.4"], "99.3,154.4" ])", caused by TSError: incorrect point given to parse, point:[object Object],[object Object]';
            expect(() => {
    ...
    describe('when given variables in a query', () => {
        it('should throw an error if variables are not supplied', () => {
            expect(() => {
                new Parser('foo:$bar');
            }).toThrow();
        });

        it('should throw an error if a varaible is an array and is not used in a xlucene function expression', () => {
            expect(() => {
                new Parser('foo:$bar', {
                    variables: {
                        other: [1, 2, 3]
        ...
        it('should throw an error if variables values contain an object', () => {
            expect(() => {
                // @ts-ignore
                new Parser('foo:$bar', {
                    variables: {
