...
describe('data_utils', () => {
    it('format isoBetween creates an iso date between two dates', (done) => {
        ...
        mocker()
            ...
            .then((data) => {
                ...
                expect(new Date(finalResult.created)).toBeBefore(new Date(opConfig.end));
                expect(new Date(finalResult.created)).toBeAfter(new Date(opConfig.start));
                done();
            })
            .catch((err) => {
                fail(err);
    ...
    it('between formats will default to current date if end is missing', (done) => {
        ...
        mocker()
            ...
            .then((data) => {
                ...
                const finalResult = data.schema[0];
                expect(new Date(finalResult.created)).toBeBefore(newDate);
                expect(new Date(finalResult.created)).toBeAfter(new Date(opConfig.start));
                done();
            })
            .catch((err) => {
    ...
    it('between formats will default to 01 January, 1970 UTC if start is missing', (done) => {
        ...
        mocker()
            ...
            .then((data) => {
                const finalResult = data.schema[0];
                expect(new Date(finalResult.created)).toBeBefore(new Date(endDate.format()));
                expect(new Date(finalResult.created)).toBeAfter(newDate);
                done();
            })
            .catch((err) => {
                fail(err);
