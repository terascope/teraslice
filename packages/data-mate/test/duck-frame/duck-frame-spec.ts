...
import { DuckFrame, DuckContext, CoercionFailureError } from '../../src/duck-frame/DuckFrame.js';
...
describe('DuckFrame', () => {
    ...
    beforeAll(async () => {
        context = await DuckContext.create();
    });
    ...
    describe('fromRecords', () => {
        it('should build a frame from JS records, the shape callers actually have', async () => {
            const frame = await DuckFrame.fromRecords(CONFIG, RECORDS, { context });
        ...
        it('should raise a CoercionFailureError naming the field and value', async () => {
            ...
            expect.assertions(4);
            try {
                await DuckFrame.fromRecords(CONFIG, bad, { context });
            } catch (err) {
        ...
        it('should null the value instead of raising in lenient mode', async () => {
            ...
            const frame = await DuckFrame.fromRecords(
                CONFIG, [{ ...RECORDS[0], bytes: 'not-a-number' }], { mode: 'lenient', context }
            );
            const rows = await context.rows(`SELECT bytes FROM ${frame.from}`);
        ...
        it('should round-trip a Long above 2^53 without losing or gaining 1', async () => {
            ...
            const frame = await DuckFrame.fromRecords(
                cfg, [{ total: '9007199254740993' }], { name: 'bigint', context }
            );
            const rows = await context.rows(`SELECT total::VARCHAR FROM ${frame.from}`);
        ...
        it('should decode a geohash, since coerceToType calls parseGeoPoint', async () => {
            ...
            const frame = await DuckFrame.fromRecords(
                cfg, [{ loc: 'ezs42' }], { name: 'geohash', context }
            );
            const rows = await context.rows(`SELECT loc.lat::VARCHAR, loc.lon::VARCHAR FROM ${frame.from}`);
            expect(rows[0].map(String)).toEqual(['42.605', '-5.603']);
            await frame.destroy();
        });

        it('should give each frame its own table', async () => {
            const a = await DuckFrame.fromRecords(CONFIG, RECORDS, { context });
            const b = await DuckFrame.fromRecords(CONFIG, RECORDS, { context });
    ...
    describe('Parquet round trip', () => {
        ...
        it('should round-trip through Parquet without re-coercing', async () => {
            const source = await DuckFrame.fromRecords(CONFIG, RECORDS, { context });
            await source.writeParquet(path);
            const restored = await DuckFrame.fromParquet(CONFIG, path, { context });

            expect(await restored.size()).toEqual(3);
            const a = await context.rows(`SELECT * FROM ${source.from} ORDER BY _key`);
            const b = await context.rows(`SELECT * FROM ${restored.from} ORDER BY _key`);
    ...
    describe('rows (streaming output path)', () => {
        ...
        it('should stream a result larger than one DuckDB chunk', async () => {
            ...
            const frame = await DuckFrame.fromRecords(CONFIG, many, { name: 'big', context });
            let count = 0;
        ...
        it('should stream an empty frame without yielding', async () => {
            const frame = await DuckFrame.fromRecords(CONFIG, RECORDS, { context });
            await context.run(`DELETE FROM ${frame.table}`);
            const out = [];
            for await (const row of frame.rows()) out.push(row);
            expect(out).toEqual([]);
            await frame.destroy();
    ...
    describe('relation vs materialized', () => {
        ...
        it('should read Parquet as a relation, copying nothing', async () => {
            ...
            const restored = await DuckFrame.fromParquet(CONFIG, p, { context });
    ...
    describe('destroy', () => {
        it('should drop the table and leave the context usable', async () => {
            const frame = await DuckFrame.fromRecords(CONFIG, RECORDS, { context });
            await frame.destroy();
            expect(await context.scalar('SELECT 1')).toBeDefined();
        });
    });
});
