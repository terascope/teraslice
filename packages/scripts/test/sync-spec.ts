    ...
    describe('->syncVersions', () => {
        describe('when the dependencies are out of sync', () => {
            ...
            it('should return 3 packages', () => {
                expect(updated).toBeArrayOfSize(3);
            });

            it('should correctly update package-1', () => {
                const pkg = updated.find(({ name }) => name === 'package-1');
                expect(pkg).toEqual({
                    name: 'package-1',
                    version: '1.0.0',
                    dependencies: {
            ...
            it('should correctly update package-2', () => {
                const pkg = updated.find(({ name }) => name === 'package-2');
                expect(pkg).toEqual({
                    name: 'package-2',
                    version: '2.0.0',
                    dependencies: {
            ...
            it('should correctly update package-3', () => {
                const pkg = updated.find(({ name }) => name === 'package-3');
                expect(pkg).toEqual({
                    name: 'package-3',
                    version: '3.0.0',
                    dependencies: {
