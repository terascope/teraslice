...
for (const pkgJsonPath of findPackageJsonFiles(APP_DIR)) {
    ...
    // "The values in peerDependencies should be either a valid semver range,
    // a `workspace:` spec, or a `catalog:` spec"
    // So we delete the entry from peerDependencies and move it into
    // dependencies so pnpm actually installs it
