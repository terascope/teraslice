...
// Generated by writeCephOsdCompose() and merged over the base file
// It holds the osd1..osdN-1 services CEPH_OSD_COUNT asks for.
const cephOsdComposeFile = path.join(cephDir, 'docker-compose.osds.yml');
const cephTlsComposeFile = path.join(cephDir, 'docker-compose.tls.yml');
const cephContainerCertDir = '/opt/certs';
    ...
    for (const service of launchServices) {
        ...
        if (service === Service.Ceph) {
            ...
            const subprocess = execa(
                ...
                [
                    'compose',
                    ...getCephComposeFiles().flatMap((file) => ['-f', file]),
                    'logs',
                    '-f',
                    '--no-color'
                ],
 ...
 *
 * The file list is derived rather than passed around, so every call site
 * (including teardown and log piping) drives the same stack: the OSD override
 * is present on disk only when writeCephOsdCompose() needed it, and the TLS
 * overlay is only merged when ENCRYPT_CEPH is set.
 */
function getCephComposeFiles(): string[] {
    const files = [cephComposeFile];
    if (fs.existsSync(cephOsdComposeFile)) {
        files.push(cephOsdComposeFile);
    }
    if (config.ENCRYPT_CEPH) {
        files.push(cephTlsComposeFile);
    }
    return files;
}

function getCephCompose(): Compose {
    return new Compose(getCephComposeFiles(), {
        env: { COMPOSE_PROJECT_NAME: cephProjectName }
    });
}

/**
 * Give the stack one OSD service per `CEPH_OSD_COUNT`.
 */
async function writeCephOsdCompose(count: number): Promise<void> {
    // Removed first so a lowered count doesn't leave the previous run's extra
    // services behind. `down --remove-orphans` still cleans up their containers.
    await fs.remove(cephOsdComposeFile);
    if (count <= 1) return;

    const base = load(await fs.readFile(cephComposeFile, 'utf8')) as {
        services?: Record<string, Record<string, unknown>>;
    };

    const osd0 = base.services?.osd0;
    if (!osd0) {
        throw new TSError(`Expected an osd0 service in ${cephComposeFile}`);
    }

    // js-yaml leaves the `<<: *ceph` merge key unresolved, and an anchor is
    // per file anyway, so flatten the shared config into each generated service.
    const { '<<': shared, ...osd } = osd0;

    const osds: Record<string, unknown> = {};
    ...
    for (let id = 1; id < count; id++) {
        ...
        osds[name] = {
            ...
            ...osd,
            hostname: `ceph-${name}`,
            environment: { OSD_ID: id }
        };
        // Merges into rgw's existing depends_on map, so RGW still waits for
        // every OSD it is going to wait_for_osds on.
        dependsOn[name] = { condition: 'service_started' };
    }

    const contents = [
        `# GENERATED by ts-scripts from ${path.basename(cephComposeFile)} to match`,
        '# CEPH_OSD_COUNT -- see writeCephOsdCompose() in',
        '# src/helpers/test-runner/services.ts. Overwritten on every run.',
        // noRefs: the services share objects with osd0, and without it they are
        // dumped as `&ref_0`/`*ref_0` anchors -- valid, but harder to read.
        dump(
            { services: { ...osds, rgw: { depends_on: dependsOn } } },
            { lineWidth: 100, noRefs: true }
        )
    ].join('\n');

    await fs.outputFile(cephOsdComposeFile, contents);
}
...
async function writeCephEnv(): Promise<void> {
    const lines = [
        ...
        `S3_ENDPOINT=${config.CEPH_PROTOCOL}://rgw:${config.CEPH_PORT}`,
        '',
        ...
        '',
        `CEPH_CERT_DIR=${config.CERT_PATH}`,
        `RGW_SSL=${config.ENCRYPT_CEPH}`,
        `RGW_SSL_CERT=${cephContainerCertDir}/ceph-keypair.pem`,
        `RGW_SSL_CA=${cephContainerCertDir}/CAs/rootCA.pem`,
        ''
    ];
...
export async function ensureCeph(options: TestOptions): Promise<() => Promise<void>> {
    ...
    await writeCephEnv();
    await writeCephOsdCompose(config.CEPH_OSD_COUNT);
    ...
    if (launchServices.includes(Service.Ceph)) {
        connectors.s3 = {
            default: {
                ...
                forcePathStyle: true,
                sslEnabled: config.ENCRYPT_CEPH,
                region: 'us-east-1',
                ...(config.ENCRYPT_CEPH && {
                    caCertificate: fs.readFileSync(
                        path.join(config.CERT_PATH, 'CAs/rootCA.pem'), 'utf8'
                    )
                })
            }
        };
