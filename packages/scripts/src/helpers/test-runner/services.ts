 ...
 *
 * The file list is read off disk rather than passed around: writeCephOsdCompose()
 * writes the OSD override when it is needed and deletes it when it is not, so
 * every call site (including teardown and log piping) drives the same stack.
 */
function getCephComposeFiles(): string[] {
    if (fs.existsSync(cephOsdComposeFile)) {
        return [cephComposeFile, cephOsdComposeFile];
    }
    return [cephComposeFile];
}
