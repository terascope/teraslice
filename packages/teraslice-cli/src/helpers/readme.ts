...
export async function updateReadmeOpList(assetBaseDir: string): Promise<void> {
    ...
    try {
        ...
        const updated = parts.slice(1).map((section) => {
            ...
            const note = '<!-- NOTE: This code is auto-generated by `teraslice-cli assets registry`.\n'
            ...
            if (headingLine.trim() === 'APIS') {
                newContent = _generateSectionContent('apis', apisSet);
            } else if (headingLine.trim() === 'Operations') {
                newContent = _generateSectionContent('operations', operationsSet);
            }
            return newContent !== undefined ? `${headingLine}\n\n${note}\n${newContent}\n\n` : section;
        });
        ...
        reply.error(`Failed to update README: ${err}`);
    }
}
...
function _generateSectionContent(section: string, set: Set<string>): string {
    return Array
        ...
        .sort()
        .map((name) => {
            const opDocPath = `./docs/asset/${section}/${name}`;
            return fs.existsSync(`${opDocPath}.md`)
                ? `- [${name}](${opDocPath})`
                : `- ${name}`;
        })
        .join('\n');
