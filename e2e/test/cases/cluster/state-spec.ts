...
describe('cluster state', () => {
    ...
    if (TEST_PLATFORM === 'native') {
            ...
            function verifyClusterState(state: any[], workersAdded = 0) {
                ...
                Object.values(state).forEach((node) => {
                    ...
                    if (actualLength !== expectActiveLength) {
                        signale.warn(
                            `Expected node.active "${node.active.length
    ...
    } else if (TEST_PLATFORM === 'kubernetesV2') {
        describe('kubernetesV2', () => {
            ...
            it('should be correct for running job with 1 worker', async () => {
                ...
                expect(state[nodes[0]].total).toBe('N/A');

                expect(state[nodes[0]].available).toBe('N/A');

                expect(findWorkers(state[nodes[0]].active, 'worker', exId)).toBeArrayOfSize(1);
                // @ts-expect-error
                expect(checkState(state, null, exId)).toBe(2);
