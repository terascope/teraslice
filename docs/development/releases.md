...
To bump the `dev` prerelease version run `pnpm bump --prerelease --prerelease-id dev teraslice [...packages]`.
Once all milestone goals have been reached the version can be bumped to an `-rc.0` tag by running `pnpm bump --premajor teraslice [...packages]`. Once QA has passed on the release candidate the version can be bumped to `v*.0.0` by running `pnpm bump --major teraslice [...packages]`. Then a PR against the master branch can be created to merge in all milestone changes.
